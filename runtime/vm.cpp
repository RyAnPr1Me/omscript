...
#define DISPATCH() \
    ...
    op_ADD: {
        ...
        if (registers[rs1].getType() == Value::Type::INTEGER &&
    ...
    op_STORE_LOCAL: {
        ...
        if (index >= locals.size()) {
            if (static_cast<size_t>(index) + 1 > kMaxLocals) {
                throw std::runtime_error("Local variable index exceeds maximum (" +
                    std::to_string(kMaxLocals) + ")");
            }
    ...
    while (ip < bytecode.size()) {
        ...
        switch (op) {
            ...
            case OpCode::DIV: case OpCode::MOD: {
                ...
                uint8_t rs2 = readByte(bytecode, ip);
                switch (op) {
                    case OpCode::ADD: registers[rd] = registers[rs1] + registers[rs2]; break;
                    case OpCode::SUB: registers[rd] = registers[rs1] - registers[rs2]; break;
                    case OpCode::MUL: registers[rd] = registers[rs1] * registers[rs2]; break;
                    case OpCode::DIV: registers[rd] = registers[rs1] / registers[rs2]; break;
                    case OpCode::MOD: registers[rd] = registers[rs1] % registers[rs2]; break;
                    default: break;
                }
                break;
            }
            ...
            case OpCode::LE: case OpCode::GT: case OpCode::GE: {
                ...
                uint8_t rs2 = readByte(bytecode, ip);
                switch (op) {
                    case OpCode::EQ: registers[rd] = Value(static_cast<int64_t>(registers[rs1] == registers[rs2])); break;
                    case OpCode::NE: registers[rd] = Value(static_cast<int64_t>(registers[rs1] != registers[rs2])); break;
                    case OpCode::LT: registers[rd] = Value(static_cast<int64_t>(registers[rs1] < registers[rs2])); break;
                    case OpCode::LE: registers[rd] = Value(static_cast<int64_t>(registers[rs1] <= registers[rs2])); break;
                    case OpCode::GT: registers[rd] = Value(static_cast<int64_t>(registers[rs1] > registers[rs2])); break;
                    case OpCode::GE: registers[rd] = Value(static_cast<int64_t>(registers[rs1] >= registers[rs2])); break;
                    default: break;
                }
                break;
            }
            ...
            case OpCode::SHL: case OpCode::SHR: {
                ...
                uint8_t rs2 = readByte(bytecode, ip);
                switch (op) {
                    case OpCode::BIT_AND: registers[rd] = registers[rs1] & registers[rs2]; break;
                    case OpCode::BIT_OR:  registers[rd] = registers[rs1] | registers[rs2]; break;
                    case OpCode::BIT_XOR: registers[rd] = registers[rs1] ^ registers[rs2]; break;
                    case OpCode::SHL:     registers[rd] = registers[rs1] << registers[rs2]; break;
                    case OpCode::SHR:     registers[rd] = registers[rs1] >> registers[rs2]; break;
                    default: break;
                }
                break;
            }
