...
static bool isAllocCall(const Expression* expr, std::string* regionVar = nullptr) {
    if (!expr || expr->type != ASTNodeType::CALL_EXPR)
        return false;
    const auto* c = static_cast<const CallExpr*>(expr);
    if (c->callee != "alloc" || c->arguments.empty())
        return false;
    if (regionVar) {
...
static bool exprUsesVar(const Expression* expr, const std::string& name) {
    ...
    case ASTNodeType::INDEX_ASSIGN_EXPR: {
        ...
        return exprUsesVar(ia->array.get(), name) || exprUsesVar(ia->index.get(), name) ||
               exprUsesVar(ia->value.get(), name);
    }
    case ASTNodeType::FIELD_ACCESS_EXPR:
        return exprUsesVar(static_cast<const FieldAccessExpr*>(expr)->object.get(), name);
    ...
    case ASTNodeType::STRUCT_LITERAL_EXPR: {
        ...
        for (const auto& fv : sl->fieldValues)
            if (exprUsesVar(fv.second.get(), name))
                return true;
        return false;
    }
    case ASTNodeType::DICT_EXPR: {
        ...
        for (const auto& p : d->pairs)
            if (exprUsesVar(p.first.get(), name) || exprUsesVar(p.second.get(), name))
                return true;
        return false;
    }
    ...
    case ASTNodeType::RANGE_ANNOT_EXPR:
        return exprUsesVar(static_cast<const RangeAnnotExpr*>(expr)->inner.get(), name);
    default:
        return false;
    }
}
...
static bool stmtUsesVar(const Statement* stmt, const std::string& name) {
    ...
    case ASTNodeType::EXPR_STMT:
        return exprUsesVar(static_cast<const ExprStmt*>(stmt)->expression.get(), name);
    case ASTNodeType::VAR_DECL:
        return exprUsesVar(static_cast<const VarDecl*>(stmt)->initializer.get(), name);
    case ASTNodeType::RETURN_STMT:
        return exprUsesVar(static_cast<const ReturnStmt*>(stmt)->value.get(), name);
    case ASTNodeType::IF_STMT: {
        const auto* is = static_cast<const IfStmt*>(stmt);
        return exprUsesVar(is->condition.get(), name) || stmtUsesVar(is->thenBranch.get(), name) ||
               stmtUsesVar(is->elseBranch.get(), name);
    }
    case ASTNodeType::WHILE_STMT: {
        const auto* ws = static_cast<const WhileStmt*>(stmt);
        return exprUsesVar(ws->condition.get(), name) || stmtUsesVar(ws->body.get(), name);
    }
    case ASTNodeType::DO_WHILE_STMT: {
        const auto* dw = static_cast<const DoWhileStmt*>(stmt);
        return stmtUsesVar(dw->body.get(), name) || exprUsesVar(dw->condition.get(), name);
    }
    case ASTNodeType::FOR_STMT: {
        const auto* fs = static_cast<const ForStmt*>(stmt);
        return exprUsesVar(fs->start.get(), name) || exprUsesVar(fs->end.get(), name) ||
               exprUsesVar(fs->step.get(), name) || stmtUsesVar(fs->body.get(), name);
    }
    case ASTNodeType::FOR_EACH_STMT: {
        const auto* fe = static_cast<const ForEachStmt*>(stmt);
        return exprUsesVar(fe->collection.get(), name) || stmtUsesVar(fe->body.get(), name);
    }
    case ASTNodeType::BLOCK: {
        const auto* b = static_cast<const BlockStmt*>(stmt);
        for (const auto& s : b->statements)
            if (stmtUsesVar(s.get(), name))
                return true;
        return false;
    }
    case ASTNodeType::SWITCH_STMT: {
        const auto* sw = static_cast<const SwitchStmt*>(stmt);
        if (exprUsesVar(sw->condition.get(), name))
            return true;
        for (const auto& cas : sw->cases) {
            if (exprUsesVar(cas.value.get(), name))
                return true;
            for (const auto& v : cas.values)
                if (exprUsesVar(v.get(), name))
                    return true;
    ...
    case ASTNodeType::THROW_STMT:
        return exprUsesVar(static_cast<const ThrowStmt*>(stmt)->value.get(), name);
    case ASTNodeType::CATCH_STMT:
        return stmtUsesVar(static_cast<const CatchStmt*>(stmt)->body.get(), name);
    case ASTNodeType::DEFER_STMT:
        return stmtUsesVar(static_cast<const DeferStmt*>(stmt)->body.get(), name);
    case ASTNodeType::ASSUME_STMT: {
        const auto* as = static_cast<const AssumeStmt*>(stmt);
        return exprUsesVar(as->condition.get(), name) || stmtUsesVar(as->deoptBody.get(), name);
    }
    case ASTNodeType::INVALIDATE_STMT:
        return static_cast<const InvalidateStmt*>(stmt)->varName == name;
    case ASTNodeType::MOVE_DECL:
        return exprUsesVar(static_cast<const MoveDecl*>(stmt)->initializer.get(), name);
    ...
    case ASTNodeType::PIPELINE_STMT: {
        ...
        if (exprUsesVar(pl->count.get(), name))
            return true;
        for (const auto& stage : pl->stages)
            if (stmtUsesVar(stage.body.get(), name))
                return true;
        return false;
    }
    default:
...
static void renameInExpr(Expression* expr, const std::unordered_map<std::string, std::string>& rmap);
static void renameInStmt(Statement* stmt, const std::unordered_map<std::string, std::string>& rmap);
...
static void renameInExpr(Expression* expr, const std::unordered_map<std::string, std::string>& rmap) {
    ...
    case ASTNodeType::FIELD_ACCESS_EXPR:
        renameInExpr(static_cast<FieldAccessExpr*>(expr)->object.get(), rmap);
        break;
    case ASTNodeType::FIELD_ASSIGN_EXPR: {
    ...
    case ASTNodeType::BORROW_EXPR:
        ...
        break;
    case ASTNodeType::REBORROW_EXPR:
        renameInExpr(static_cast<ReborrowExpr*>(expr)->source.get(), rmap);
        break;
    case ASTNodeType::SPREAD_EXPR:
        renameInExpr(static_cast<SpreadExpr*>(expr)->operand.get(), rmap);
        break;
    case ASTNodeType::PIPE_EXPR:
...
static void renameInStmt(Statement* stmt, const std::unordered_map<std::string, std::string>& rmap) {
    ...
    switch (stmt->type) {
    case ASTNodeType::EXPR_STMT:
        renameInExpr(static_cast<ExprStmt*>(stmt)->expression.get(), rmap);
        break;
    case ASTNodeType::VAR_DECL: {
    ...
    case ASTNodeType::RETURN_STMT:
        renameInExpr(static_cast<ReturnStmt*>(stmt)->value.get(), rmap);
        break;
    case ASTNodeType::IF_STMT: {
    ...
    case ASTNodeType::SWITCH_STMT: {
        ...
        for (auto& cas : sw->cases) {
            ...
            for (auto& v : cas.values)
                renameInExpr(v.get(), rmap);
            for (auto& s : cas.body)
                renameInStmt(s.get(), rmap);
        }
        break;
    ...
    case ASTNodeType::THROW_STMT:
        renameInExpr(static_cast<ThrowStmt*>(stmt)->value.get(), rmap);
        break;
    case ASTNodeType::CATCH_STMT:
        renameInStmt(static_cast<CatchStmt*>(stmt)->body.get(), rmap);
        break;
    case ASTNodeType::DEFER_STMT:
        renameInStmt(static_cast<DeferStmt*>(stmt)->body.get(), rmap);
        break;
    case ASTNodeType::ASSUME_STMT: {
    ...
    case ASTNodeType::INVALIDATE_STMT:
        ...
        break;
    case ASTNodeType::MOVE_DECL:
        renameInExpr(static_cast<MoveDecl*>(stmt)->initializer.get(), rmap);
        break;
    case ASTNodeType::FREEZE_STMT:
    ...
    case ASTNodeType::PREFETCH_STMT: {
        ...
        rename(ps->varName);
        if (ps->varDecl)
            renameInExpr(ps->varDecl->initializer.get(), rmap);
        break;
    }
    case ASTNodeType::PIPELINE_STMT: {
        ...
        for (auto& stage : pl->stages)
            renameInStmt(stage.body.get(), rmap);
        break;
    }
    default:
        break;
...
static std::string canonical(const std::string& name, const std::unordered_map<std::string, std::string>& rmap) {
...
static void processBlock(std::vector<std::unique_ptr<Statement>>& stmts, const std::string& funcName, RLCStats& stats,
                         bool verbose) {
    ...
    for (const auto& sp : stmts) {
        if (!sp || sp->type != ASTNodeType::RETURN_STMT)
            continue;
    ...
    for (const auto& ri : regions) {
        if (ri.isGlobal)
            continue; // globals are exempt
        if (returnedVars.count(ri.name))
            continue; // returned → exempt
    ...
    for (int i = 0; i < static_cast<int>(regions.size()); ++i) {
        ...
        if (verbose) {
            std::cout << "  [rlc] " << funcName << ": coalesced '" << r2.name << "' into '" << r1Canon << "'\n";
        }
    }
    ...
    for (int i = 0; i < n; ++i) {
        if (removedStmts.count(i))
            continue;
        const Statement* s = stmts[i].get();
        if (!s || s->type != ASTNodeType::EXPR_STMT)
            continue;
        const auto* es = static_cast<const ExprStmt*>(s);
        std::string rv;
        if (isAllocCall(es->expression.get(), &rv)) {
            if (renameMap.count(rv))
                ++stats.allocsRedirected;
        }
    }
...
RLCStats runRLCPass(Program* program, bool verbose) {
    ...
    forEachFunction(program, [&](FunctionDecl* func) {
        ...
        for (int iter = 0; iter < kMaxCoalescingIterations; ++iter) {
            ...
            if (stats.regionsCoalesced == before)
                break;
        }
    });

    if (verbose && stats.regionsCoalesced > 0) {
        std::cout << "[rlc] Total: " << stats.regionsCoalesced << " region pair(s) coalesced, "
                  << stats.allocsRedirected << " alloc(s) redirected, " << stats.invalidatesRemoved
                  << " invalidate(s) removed\n";
    }
    return stats;
}
