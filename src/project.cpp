...
static void applyProfileKey(BuildProfile& p, const std::string& key, const std::string& rawVal) {
    const std::string val = trim(stripInlineComment(rawVal));
    int64_t n = 0;
    bool b = false;

    if (key == "opt_level") {
        if (parseTomlInt(val, n))
            p.optLevel = intToOptLevel(n);
    } else if (key == "egraph") {
        if (parseTomlBool(val, b))
            p.egraph = b;
        ...
        if (parseTomlInt(val, n))
            p.superoptLevel = static_cast<unsigned>(n);
...
OmsManifest parseOmsToml(const std::string& path) {
    ...
    while (std::getline(f, line)) {
        ...
        if (t.front() == '[' && t.back() == ']') {
            ...
            if (section.rfind("profile.", 0) == 0) {
                ...
                if (!manifest.profiles.count(currentProfile)) {
                    manifest.profiles[currentProfile] = BuildProfile::makeRelease();
                    manifest.profiles[currentProfile].outDir = "target/" + currentProfile;
                }
            } else {
        ...
        if (section == "project") {
            std::string sv;
            if (key == "name" && parseTomlString(val, sv))
                manifest.name = sv;
            if (key == "version" && parseTomlString(val, sv))
                manifest.version = sv;
            if (key == "entry" && parseTomlString(val, sv))
                manifest.entry = sv;
    ...
    while (true) {
        ...
        if (fs::exists(candidate, ec) && !ec) {
            try {
                ...
                std::cerr << "Warning: found oms.toml but failed to parse: " << ex.what() << "\n";
                return std::nullopt;
            }
        }
...
ProjectContext makeEphemeralProject(const std::string& sourceFile, const std::string& /*profileName*/) {
    namespace fs = std::filesystem;
    ProjectContext ctx;
    std::error_code ec;
...
bool initProject(const std::string& dir, const std::string& name) {
    ...
    if (ec) {
        std::cerr << "Error: cannot resolve directory '" << dir << "': " << ec.message() << "\n";
        return false;
    }

    const auto manifestPath = absDir / "oms.toml";
    if (fs::exists(manifestPath, ec)) {
        std::cerr << "Error: oms.toml already exists in " << absDir.string() << "\n";
        return false;
    }
    ...
    if (ec) {
        std::cerr << "Error: cannot create '" << (absDir / "src").string() << "': " << ec.message() << "\n";
        return false;
    }
    ...
    std::cout << "Created project '" << name << "' at " << absDir.string() << "\n"
              << "  oms.toml\n"
              << "  src/main.om\n"
