...
static bool stmtCallsAny(const Statement* s, const std::unordered_set<std::string>& names) {
    ...
    case ASTNodeType::SWITCH_STMT: {
        ...
        if (exprCallsAny(sw->condition.get(), names))
            return true;
        for (const auto& sc : sw->cases) {
            if (sc.value && exprCallsAny(sc.value.get(), names))
                return true;
            for (const auto& val : sc.values)
                if (exprCallsAny(val.get(), names))
                    return true;
...
static bool isUnsignedAnnotation(const std::string& tn) {
    ...
    if (tn.size() >= 2 && tn[0] == 'u') {
        for (size_t j = 1; j < tn.size(); ++j)
            if (!std::isdigit(static_cast<unsigned char>(tn[j])))
                return false;
...
static const std::unordered_set<std::string> stdlibFunctions = {
    "abs", "acos", "array_any", "array_concat", "array_contains", "array_copy", "array_count", "array_drop",
    "array_every", "array_fill", "array_filter", "array_find", "array_insert", "array_last", "array_map", "array_max",
    "array_mean", "array_min", "array_product", "array_reduce", "array_remove", "array_rotate", "array_slice",
    "array_take", "array_unique", "asin", "assert", "assume", "alloc", "atan", "atan2", "bitreverse", "bswap", "cbrt",
    "ceil", "char_at", "char_code", "clamp", "clz", "command", "copysign", "cos", "ctz", "exit_program", "exit", "exp",
    "exp2", "expect", "fast_add", "fast_div", "fast_mul", "fast_sub", "file_append", "file_exists", "file_read",
    "file_write", "filter", "floor", "fma", "gcd", "hypot", "index_of", "input", "input_line", "is_alnum", "is_alpha",
    "is_digit", "is_even", "is_lower", "is_odd", "is_power_of_2", "is_space", "is_upper", "lcm", "len", "log", "log10",
    "log2", "map_filter", "map_get", "map_has", "map_invert", "map_keys", "map_merge", "map_new", "map_remove",
    "map_set", "map_size", "map_values", "max", "max_float", "min", "min_float", "number_to_string", "newRegion", "pop",
    "popcount", "pow", "precise_add", "precise_div", "precise_mul", "precise_sub", "print", "print_char", "println",
    "push", "random", "range", "range_step", "reverse", "rotate_left", "rotate_right", "round", "saturating_add",
    "saturating_sub", "shell", "sign", "sin", "sleep", "sort", "sqrt", "str_chars", "str_concat", "str_contains",
    "str_count", "str_ends_with", "str_eq", "str_filter", "str_find", "str_index_of", "str_join", "str_len",
...
void CodeGenerator::initTBAAMetadata() {
    ...
    auto makeTBAAType = [&](const char* name) -> llvm::MDNode* {
        return llvm::MDNode::get(C,
                                 {llvm::MDString::get(C, name), tbaaRoot_,
                                  llvm::ConstantAsMetadata::get(llvm::ConstantInt::get(llvm::Type::getInt64Ty(C), 0))});
    };
    ...
    auto* zero = llvm::ConstantAsMetadata::get(llvm::ConstantInt::get(llvm::Type::getInt64Ty(C), 0));
    ...
    arrayLenRangeMD_ = llvm::MDNode::get(
        C, {llvm::ConstantAsMetadata::get(llvm::ConstantInt::get(i64Ty, 0)),
            llvm::ConstantAsMetadata::get(llvm::ConstantInt::get(i64Ty, static_cast<uint64_t>(INT64_MAX)))});
    // !range [0, 2): boolean i64 results (is_alpha, is_digit, str_eq, etc.)
    boolRangeMD_ = llvm::MDNode::get(C, {llvm::ConstantAsMetadata::get(llvm::ConstantInt::get(i64Ty, 0)),
                                         llvm::ConstantAsMetadata::get(llvm::ConstantInt::get(i64Ty, 2))});
    // !range [0, 256): char i64 results (char_at)
    charRangeMD_ = llvm::MDNode::get(C, {llvm::ConstantAsMetadata::get(llvm::ConstantInt::get(i64Ty, 0)),
                                         llvm::ConstantAsMetadata::get(llvm::ConstantInt::get(i64Ty, 256))});
    // !range [0, 65): bit-count i64 results (popcount/clz/ctz).
    bitcountRangeMD_ = llvm::MDNode::get(C, {llvm::ConstantAsMetadata::get(llvm::ConstantInt::get(i64Ty, 0)),
...
llvm::Value* CodeGenerator::emitStringCap(llvm::Value* strPtr, const llvm::Twine& name) {
    ...
    auto* capPtr =
        builder->CreateInBoundsGEP(llvm::Type::getInt64Ty(*context), strPtr,
...
llvm::Value* CodeGenerator::emitStringData(llvm::Value* strPtr, const llvm::Twine& name) {
    ...
    return builder->CreateInBoundsGEP(llvm::Type::getInt8Ty(*context), strPtr,
...
llvm::Value* CodeGenerator::emitAllocString(llvm::Value* len, llvm::Value* cap, const llvm::Twine& name) {
...
llvm::MDNode* CodeGenerator::getOrCreateFieldTBAA(const std::string& structType, size_t fieldIdx) {
    ...
    if (it != tbaaStructFieldCache_.end())
        return it->second;
    ...
    llvm::MDNode* fieldTypeNode = llvm::MDNode::get(C, {llvm::MDString::get(C, nodeName), tbaaStructTypeNode_, zero});
...
llvm::Type* CodeGenerator::resolveAnnotatedType(const std::string& annotation) {
    ...
    for (const SimdTypeRow& r : kSimdTypeRegistry) {
        if (ann == r.name) {
            llvm::Type* elemTy =
                r.isFloat ? (r.bits == 32 ? llvm::Type::getFloatTy(*context) : llvm::Type::getDoubleTy(*context))
...
llvm::StructType* CodeGenerator::getOrCreateStructLLVMType(const std::string& name) {
    ...
    if (declIt != structFieldDecls_.end() && !declIt->second.empty()) {
        ...
        for (const auto& fd : declIt->second) {
            llvm::Type* t = fd.typeName.empty() ? getDefaultType() : resolveAnnotatedType(fd.typeName);
    ...
    llvm::StructType* sty = llvm::StructType::create(*context, elemTypes, "omsc.struct." + name, isPacked);
...
llvm::Value* CodeGenerator::liftFieldLoad(llvm::Value* v, const std::string& annot) {
    ...
    if (ty->isIntegerTy()) {
        ...
        if (bits < 64) {
            ...
            return isUnsigned ? builder->CreateZExt(v, getDefaultType(), "field.zext")
...
llvm::Value* CodeGenerator::splatScalarToVector(llvm::Value* scalar, llvm::Type* vecTy) {
    ...
    llvm::Value* ins = builder->CreateInsertElement(
        poison, scalar, llvm::ConstantInt::get(llvm::Type::getInt32Ty(*context), 0), "splat.ins");
...
void CodeGenerator::bindVariableAnnotated(const std::string& name, llvm::Value* value, const std::string& typeAnnot,
                                          bool isConst) {
...
bool CodeGenerator::isUnsignedAnnot(const std::string& annot) {
    ...
    if (annot.size() >= 2 && annot[0] == 'u') {
        for (size_t j = 1; j < annot.size(); ++j)
            if (!std::isdigit(static_cast<unsigned char>(annot[j])))
                return false;
        return true;
    }
    return false;
...
bool CodeGenerator::isUnsignedValue(llvm::Value* v) const {
    ...
    while (base) {
        ...
        if (auto* cast = llvm::dyn_cast<llvm::CastInst>(base)) {
            if (cast->getOpcode() == llvm::Instruction::ZExt || cast->getOpcode() == llvm::Instruction::BitCast) {
...
void CodeGenerator::checkConstModification(const std::string& name, const std::string& action) {
    ...
    if (bs && bs->immutBorrowCount > 0) {
        throw DiagnosticError(Diagnostic{DiagnosticSeverity::Error,
                                         {"", 0, 0},
                                         "Cannot " + action + " variable '" + name + "' — it has " +
...
void CodeGenerator::attachLoopMetadataVec(llvm::BranchInst* backEdgeBr, unsigned interleaveCount) {
    if (optimizationLevel < OptimizationLevel::O1)
        return;
        ...
        if (interleaveCount > 0) {
            mds.push_back(llvm::MDNode::get(*context, {llvm::MDString::get(*context, "llvm.loop.interleave.count"),
                                                       llvm::ConstantAsMetadata::get(llvm::ConstantInt::get(
                                                           llvm::Type::getInt32Ty(*context), interleaveCount))}));
        }
    }
    llvm::MDNode* md = llvm::MDNode::get(*context, mds);
    md->replaceOperandWith(0, md);
...
CodeGenerator::CountingLoopInfo
CodeGenerator::emitCountingLoop(llvm::StringRef prefix, llvm::Value* limit, llvm::Value* start,
                                unsigned interleaveCount,
                                const std::function<void(llvm::PHINode*, llvm::BasicBlock*)>& bodyFn) {
    llvm::Function* function = builder->GetInsertBlock()->getParent();
    llvm::BasicBlock* preheader = builder->GetInsertBlock();

    llvm::BasicBlock* loopBB = llvm::BasicBlock::Create(*context, llvm::Twine(prefix) + ".loop", function);
    llvm::BasicBlock* bodyBB = llvm::BasicBlock::Create(*context, llvm::Twine(prefix) + ".body", function);
    llvm::BasicBlock* doneBB = llvm::BasicBlock::Create(*context, llvm::Twine(prefix) + ".done", function);
    ...
    llvm::PHINode* idx = builder->CreatePHI(getDefaultType(), 2, llvm::Twine(prefix) + ".idx");
...
llvm::Value* CodeGenerator::emitLoadArrayLen(llvm::Value* arrPtr, const llvm::Twine& name) {
    auto* load = builder->CreateAlignedLoad(getDefaultType(), arrPtr, llvm::MaybeAlign(8), name);
...
llvm::LoadInst* CodeGenerator::emitLoadArrayElem(llvm::Value* elemPtr, const llvm::Twine& name) {
    auto* load = builder->CreateAlignedLoad(getDefaultType(), elemPtr, llvm::MaybeAlign(8), name);
...
llvm::StoreInst* CodeGenerator::emitStoreArrayElem(llvm::Value* val, llvm::Value* elemPtr) {
    ...
    if (val->getType()->isIntegerTy() && !val->getType()->isIntegerTy(64)) {
        ...
        val = isUnsigned ? builder->CreateZExt(val, getDefaultType(), "arr.elem.zext")
...
llvm::Value* CodeGenerator::emitAllocArray(llvm::Value* len, const llvm::Twine& name) {
    ...
    llvm::Value* bytes = builder->CreateMul(slots, eight, name + ".bytes",
                                            /*NUW=*/true, /*NSW=*/true);
    llvm::Value* buf = builder->CreateCall(getOrDeclareMalloc(), {bytes}, name + ".buf");
    ...
    llvm::cast<llvm::CallInst>(buf)->addRetAttr(llvm::Attribute::getWithDereferenceableBytes(*context, derefBytes));
    emitStoreArrayLen(len, buf);
    return buf;
}

llvm::Value* CodeGenerator::emitToArrayPtr(llvm::Value* val, const llvm::Twine& name) {
    val = toDefaultType(val);
    return builder->CreateIntToPtr(val, llvm::PointerType::getUnqual(*context), name);
}
...
llvm::Function* CodeGenerator::getOrDeclareStrlen() {
    ...
    fn->addFnAttr(
        llvm::Attribute::getWithMemoryEffects(*context, llvm::MemoryEffects::argMemOnly(llvm::ModRefInfo::Ref)));
...
llvm::Function* CodeGenerator::getOrDeclareMalloc() {
    ...
    fn->addFnAttr(
        llvm::Attribute::get(*context, llvm::Attribute::AllocKind,
                             static_cast<uint64_t>(llvm::AllocFnKind::Alloc | llvm::AllocFnKind::Uninitialized)));
...
llvm::Function* CodeGenerator::getOrDeclareAlignedAlloc() {
    ...
    llvm::Function* fn = llvm::Function::Create(ty, llvm::Function::ExternalLinkage, "aligned_alloc", module.get());
    fn->addFnAttr(llvm::Attribute::NoUnwind);
    fn->addFnAttr(llvm::Attribute::WillReturn);
    fn->addFnAttr(llvm::Attribute::NoBuiltin);
    fn->addFnAttr(llvm::Attribute::NoFree);
...
llvm::Function* CodeGenerator::getOrDeclareCalloc() {
    ...
    fn->addFnAttr(llvm::Attribute::get(*context, llvm::Attribute::AllocKind,
                                       static_cast<uint64_t>(llvm::AllocFnKind::Alloc | llvm::AllocFnKind::Zeroed)));
...
llvm::Function* CodeGenerator::getOrDeclarePuts() {
    if (auto* fn = module->getFunction("puts"))
        return fn;
    auto* ty =
        llvm::FunctionType::get(llvm::Type::getInt32Ty(*context), {llvm::PointerType::getUnqual(*context)}, false);
...
llvm::Function* CodeGenerator::getOrDeclareFree() {
    ...
    fn->addFnAttr(
        llvm::Attribute::get(*context, llvm::Attribute::AllocKind, static_cast<uint64_t>(llvm::AllocFnKind::Free)));
...
llvm::Function* CodeGenerator::getOrDeclareStrdup() {
    ...
    fn->addFnAttr(llvm::Attribute::getWithMemoryEffects(
        *context, llvm::MemoryEffects(llvm::MemoryEffects::argMemOnly(llvm::ModRefInfo::Ref) |
...
llvm::Function* CodeGenerator::getOrDeclareTimeFunc() {
    ...
    fn->addFnAttr(llvm::Attribute::getWithMemoryEffects(
        *context, llvm::MemoryEffects::inaccessibleMemOnly(llvm::ModRefInfo::Ref)));
    return fn;
}
...
llvm::Function* CodeGenerator::getOrDeclareRealloc() {
    ...
    fn->addFnAttr(
        llvm::Attribute::get(*context, llvm::Attribute::AllocKind, static_cast<uint64_t>(llvm::AllocFnKind::Realloc)));
...
llvm::Function* CodeGenerator::getOrDeclarePthreadMutexInit() {
    ...
    auto* ty = llvm::FunctionType::get(llvm::Type::getInt32Ty(*context), {ptrTy, ptrTy}, false);
    llvm::Function* fn =
        llvm::Function::Create(ty, llvm::Function::ExternalLinkage, "pthread_mutex_init", module.get());
    fn->addFnAttr(llvm::Attribute::NoUnwind);
    fn->addFnAttr(llvm::Attribute::NoFree);
    OMSC_ADD_NOCAPTURE(fn, 0);
    OMSC_ADD_NOCAPTURE(fn, 1);
...
llvm::Function* CodeGenerator::getOrDeclarePthreadMutexUnlock() {
    ...
    auto* ty = llvm::FunctionType::get(llvm::Type::getInt32Ty(*context), {ptrTy}, false);
    llvm::Function* fn =
        llvm::Function::Create(ty, llvm::Function::ExternalLinkage, "pthread_mutex_unlock", module.get());
    fn->addFnAttr(llvm::Attribute::NoUnwind);
    fn->addFnAttr(llvm::Attribute::NoFree);
    OMSC_ADD_NOCAPTURE(fn, 0);
    fn->addParamAttr(0, llvm::Attribute::NonNull);
...
llvm::Function* CodeGenerator::getOrDeclarePthreadMutexDestroy() {
    ...
    auto* ty = llvm::FunctionType::get(llvm::Type::getInt32Ty(*context), {ptrTy}, false);
    llvm::Function* fn =
        llvm::Function::Create(ty, llvm::Function::ExternalLinkage, "pthread_mutex_destroy", module.get());
    fn->addFnAttr(llvm::Attribute::NoUnwind);
    fn->addFnAttr(llvm::Attribute::NoFree);
    OMSC_ADD_NOCAPTURE(fn, 0);
    fn->addParamAttr(0, llvm::Attribute::NonNull);
...
llvm::Function* CodeGenerator::getOrDeclareBigintNewI64() {
    if (auto* fn = module->getFunction("omsc_bigint_new_i64"))
        return fn;
    auto* i64Ty = llvm::Type::getInt64Ty(*context);
    auto* ptrTy = llvm::PointerType::getUnqual(*context);
...
llvm::Function* CodeGenerator::getOrDeclareBigintNewStr() {
    if (auto* fn = module->getFunction("omsc_bigint_new_str"))
        return fn;
    auto* ptrTy = llvm::PointerType::getUnqual(*context);
    auto* ty = llvm::FunctionType::get(ptrTy, {ptrTy}, false);
...
llvm::Function* CodeGenerator::getOrDeclareBigintFree() {
    if (auto* fn = module->getFunction("omsc_bigint_free"))
        return fn;
    auto* ptrTy = llvm::PointerType::getUnqual(*context);
    auto* ty = llvm::FunctionType::get(llvm::Type::getVoidTy(*context), {ptrTy}, false);
    auto* fn = llvm::Function::Create(ty, llvm::Function::ExternalLinkage, "omsc_bigint_free", module.get());
    fn->addFnAttr(llvm::Attribute::NoUnwind);
    return fn;
}

llvm::Function* CodeGenerator::getOrDeclareBigintAdd() {
    if (auto* fn = module->getFunction("omsc_bigint_add"))
        return fn;
...
llvm::Function* CodeGenerator::getOrDeclareBigintSub() {
    if (auto* fn = module->getFunction("omsc_bigint_sub"))
        return fn;
...
llvm::Function* CodeGenerator::getOrDeclareBigintMul() {
    if (auto* fn = module->getFunction("omsc_bigint_mul"))
        return fn;
...
llvm::Function* CodeGenerator::getOrDeclareBigintDiv() {
    if (auto* fn = module->getFunction("omsc_bigint_div"))
        return fn;
...
llvm::Function* CodeGenerator::getOrDeclareBigintMod() {
    if (auto* fn = module->getFunction("omsc_bigint_mod"))
        return fn;
...
llvm::Function* CodeGenerator::getOrDeclareBigintNeg() {
    if (auto* fn = module->getFunction("omsc_bigint_neg"))
        return fn;
...
llvm::Function* CodeGenerator::getOrDeclareBigintAbs() {
    if (auto* fn = module->getFunction("omsc_bigint_abs"))
        return fn;
...
llvm::Function* CodeGenerator::getOrDeclareBigintPow() {
    if (auto* fn = module->getFunction("omsc_bigint_pow"))
        return fn;
...
llvm::Function* CodeGenerator::getOrDeclareBigintGcd() {
    if (auto* fn = module->getFunction("omsc_bigint_gcd"))
        return fn;
...
llvm::Function* CodeGenerator::getOrDeclareBigintToI64() {
    if (auto* fn = module->getFunction("omsc_bigint_to_i64"))
        return fn;
    auto* ptrTy = llvm::PointerType::getUnqual(*context);
...
llvm::Function* CodeGenerator::getOrDeclareBigintBitLength() {
    if (auto* fn = module->getFunction("omsc_bigint_bit_length"))
        return fn;
    auto* ptrTy = llvm::PointerType::getUnqual(*context);
    auto* ty = llvm::FunctionType::get(llvm::Type::getInt64Ty(*context), {ptrTy}, false);
...
llvm::Function* CodeGenerator::getOrDeclareBigintIsZero() {
    if (auto* fn = module->getFunction("omsc_bigint_is_zero"))
        return fn;
    auto* ptrTy = llvm::PointerType::getUnqual(*context);
    auto* i32Ty = llvm::Type::getInt32Ty(*context);
...
llvm::Function* CodeGenerator::getOrDeclareBigintIsNegative() {
    if (auto* fn = module->getFunction("omsc_bigint_is_negative"))
        return fn;
    auto* ptrTy = llvm::PointerType::getUnqual(*context);
    auto* i32Ty = llvm::Type::getInt32Ty(*context);
    auto* ty = llvm::FunctionType::get(i32Ty, {ptrTy}, false);
...
llvm::Function* CodeGenerator::getOrDeclareBigintShl() {
    if (auto* fn = module->getFunction("omsc_bigint_shl"))
        return fn;
    auto* ptrTy = llvm::PointerType::getUnqual(*context);
    auto* i64Ty = llvm::Type::getInt64Ty(*context);
...
llvm::Function* CodeGenerator::getOrDeclareBigintShr() {
    if (auto* fn = module->getFunction("omsc_bigint_shr"))
        return fn;
    auto* ptrTy = llvm::PointerType::getUnqual(*context);
    auto* i64Ty = llvm::Type::getInt64Ty(*context);
    auto* ty = llvm::FunctionType::get(ptrTy, {ptrTy, i64Ty}, false);
...
llvm::Function* CodeGenerator::getOrDeclareHttpGet() {
    if (auto* fn = module->getFunction("omsc_http_get"))
        return fn;
    auto* ptrTy = llvm::PointerType::getUnqual(*context);
    auto* ty = llvm::FunctionType::get(ptrTy, {ptrTy}, false);
...
llvm::Function* CodeGenerator::getOrDeclareHttpPost() {
    if (auto* fn = module->getFunction("omsc_http_post"))
        return fn;
    auto* ptrTy = llvm::PointerType::getUnqual(*context);
...
llvm::Function* CodeGenerator::getOrDeclareHttpRequest() {
    if (auto* fn = module->getFunction("omsc_http_request"))
        return fn;
    auto* ptrTy = llvm::PointerType::getUnqual(*context);
...
llvm::Function* CodeGenerator::getOrDeclareHttpGetStatus() {
    if (auto* fn = module->getFunction("omsc_http_get_status"))
        return fn;
    auto* ptrTy = llvm::PointerType::getUnqual(*context);
    auto* i64Ty = llvm::Type::getInt64Ty(*context);
...
llvm::Value* CodeGenerator::emitKeyHash(llvm::Value* key) {
    ...
    llvm::Value* h = builder->CreateMul(key, llvm::ConstantInt::get(i64Ty, 0xD6E8FEB86659FD93ULL), "h.mul");

    // Step 2: rotate right by 37 (prime, coprime to 64) — lossless permutation.
    llvm::Function* fshr = OMSC_GET_INTRINSIC(module.get(), llvm::Intrinsic::fshr, {i64Ty});
    llvm::Value* rotated = builder->CreateCall(fshr, {h, h, llvm::ConstantInt::get(i64Ty, 37)}, "h.rot");
...
llvm::Function* CodeGenerator::getOrEmitHashMapSet() {
    ...
    {
        llvm::Metadata* rangeMD[] = {llvm::ConstantAsMetadata::get(llvm::ConstantInt::get(i64Ty, 8)),
                                     llvm::ConstantAsMetadata::get(llvm::ConstantInt::get(i64Ty, 1ULL << 62))};
        capLoad->setMetadata(llvm::LLVMContext::MD_range, llvm::MDNode::get(*context, rangeMD));
        llvm::Value* capM1 = builder->CreateSub(cap, llvm::ConstantInt::get(i64Ty, 1));
        llvm::Value* isPow2 =
            builder->CreateICmpEQ(builder->CreateAnd(cap, capM1), llvm::ConstantInt::get(i64Ty, 0), "cap.pow2");
    ...
    { // Size is always in [0, 2^62).
        llvm::Metadata* rangeMD[] = {llvm::ConstantAsMetadata::get(llvm::ConstantInt::get(i64Ty, 0)),
                                     llvm::ConstantAsMetadata::get(llvm::ConstantInt::get(i64Ty, 1ULL << 62))};
        sizeLoad->setMetadata(llvm::LLVMContext::MD_range, llvm::MDNode::get(*context, rangeMD));
    }
    llvm::Value* size = sizeLoad;
    llvm::Value* mask =
        builder->CreateSub(cap, llvm::ConstantInt::get(i64Ty, 1), "mask", /*HasNUW=*/true, /*HasNSW=*/true);
    ...
    llvm::Value* entryOff =
        builder->CreateAdd(builder->CreateMul(slot, three, "slot3", /*HasNUW=*/true, /*HasNSW=*/true),
    ...
    llvm::Value* newTomb =
        builder->CreateSelect(builder->CreateAnd(isTomb, builder->CreateNot(hasTomb)), slot, firstTombstone, "newtomb");
    ...
    llvm::Value* keyOff =
        builder->CreateAdd(entryOff, llvm::ConstantInt::get(i64Ty, 1), "keyoff", /*HasNUW=*/true, /*HasNSW=*/true);
    ...
    llvm::Value* valOff =
        builder->CreateAdd(entryOff, llvm::ConstantInt::get(i64Ty, 2), "valoff", /*HasNUW=*/true, /*HasNSW=*/true);
    ...
    llvm::Value* nextSlot = builder->CreateAnd(
        builder->CreateAdd(slot, llvm::ConstantInt::get(i64Ty, 1), "slot1", /*HasNUW=*/true, /*HasNSW=*/true), mask,
        "nextslot");
    ...
    llvm::Value* newSize =
        builder->CreateAdd(size, llvm::ConstantInt::get(i64Ty, 1), "newsize", /*HasNUW=*/true, /*HasNSW=*/true);
    llvm::Value* threshold = builder->CreateLShr(
        builder->CreateMul(cap, llvm::ConstantInt::get(i64Ty, 3), "cap3thr", /*HasNUW=*/true, /*HasNSW=*/true),
        llvm::ConstantInt::get(i64Ty, 2), "threshold");
    ...
    llvm::Value* hasTombForInsert =
        builder->CreateICmpNE(firstTombstone, llvm::ConstantInt::get(i64Ty, -1), "hastomb2");
    llvm::Value* insSlot = builder->CreateSelect(hasTombForInsert, firstTombstone, slot, "insslot");
    llvm::Value* insOff =
        builder->CreateAdd(builder->CreateMul(insSlot, three, "ins3", /*HasNUW=*/true, /*HasNSW=*/true),
    ...
    llvm::Value* insKeyPtr = builder->CreateInBoundsGEP(
        i64Ty, mapArg,
        builder->CreateAdd(insOff, llvm::ConstantInt::get(i64Ty, 1), "inskeyoff", /*HasNUW=*/true, /*HasNSW=*/true),
        "inskeyptr");
    {
        auto* st = builder->CreateAlignedStore(keyArg, insKeyPtr, llvm::MaybeAlign(8));
        st->setMetadata(llvm::LLVMContext::MD_tbaa, tbaaMapKey_);
    }
    llvm::Value* insValPtr = builder->CreateInBoundsGEP(
        i64Ty, mapArg,
        builder->CreateAdd(insOff, llvm::ConstantInt::get(i64Ty, 2), "insvaloff", /*HasNUW=*/true, /*HasNSW=*/true),
        "insvalptr");
    {
    ...
    llvm::Value* newCap =
        builder->CreateShl(cap, llvm::ConstantInt::get(i64Ty, 1), "newcap", /*HasNUW=*/true, /*HasNSW=*/true);
    llvm::Value* newTotalSlots =
        builder->CreateAdd(builder->CreateMul(newCap, three, "cap3", /*HasNUW=*/true, /*HasNSW=*/true),
    ...
    llvm::Value* newBuf =
        builder->CreateCall(getOrDeclareCalloc(), {llvm::ConstantInt::get(i64Ty, 1), newTotalBytes}, "newbuf");
    ...
    llvm::Value* newMask =
        builder->CreateSub(newCap, llvm::ConstantInt::get(i64Ty, 1), "newmask", /*HasNUW=*/true, /*HasNSW=*/true);
    ...
    llvm::Value* oldOff =
        builder->CreateAdd(builder->CreateMul(ri, three, "ri3", /*HasNUW=*/true, /*HasNSW=*/true),
    ...
    llvm::Value* oldKeyPtr = builder->CreateInBoundsGEP(
        i64Ty, mapArg,
        builder->CreateAdd(oldOff, llvm::ConstantInt::get(i64Ty, 1), "oldkeyoff", /*HasNUW=*/true, /*HasNSW=*/true),
        "oldkeyptr");
    ...
    llvm::Value* oldValPtr = builder->CreateInBoundsGEP(
        i64Ty, mapArg,
        builder->CreateAdd(oldOff, llvm::ConstantInt::get(i64Ty, 2), "oldvaloff", /*HasNUW=*/true, /*HasNSW=*/true),
        "oldvalptr");
    ...
    llvm::Value* rEntryOff =
        builder->CreateAdd(builder->CreateMul(rs, three, "rs3", /*HasNUW=*/true, /*HasNSW=*/true),
    ...
    llvm::Value* rNext = builder->CreateAnd(
        builder->CreateAdd(rs, llvm::ConstantInt::get(i64Ty, 1), "rs1", /*HasNUW=*/true, /*HasNSW=*/true), newMask,
        "rnext");
    ...
    llvm::Value* rKeyPtr = builder->CreateInBoundsGEP(
        i64Ty, newBuf,
        builder->CreateAdd(rEntryOff, llvm::ConstantInt::get(i64Ty, 1), "rkeyoff", /*HasNUW=*/true, /*HasNSW=*/true),
        "rkeyptr");
    {
        auto* st = builder->CreateAlignedStore(oldKey, rKeyPtr, llvm::MaybeAlign(8));
        st->setMetadata(llvm::LLVMContext::MD_tbaa, tbaaMapKey_);
    }
    llvm::Value* rValPtr = builder->CreateInBoundsGEP(
        i64Ty, newBuf,
        builder->CreateAdd(rEntryOff, llvm::ConstantInt::get(i64Ty, 2), "rvaloff", /*HasNUW=*/true, /*HasNSW=*/true),
        "rvalptr");
    {
    ...
    llvm::Value* riNext =
        builder->CreateAdd(ri, llvm::ConstantInt::get(i64Ty, 1), "rinext", /*HasNUW=*/true, /*HasNSW=*/true);
    ...
    llvm::Value* nEntryOff =
        builder->CreateAdd(builder->CreateMul(ns, three, "ns3", /*HasNUW=*/true, /*HasNSW=*/true),
    ...
    llvm::Value* nNext = builder->CreateAnd(
        builder->CreateAdd(ns, llvm::ConstantInt::get(i64Ty, 1), "ns1", /*HasNUW=*/true, /*HasNSW=*/true), newMask,
        "nnext");
    ...
    llvm::Value* nKeyPtr = builder->CreateInBoundsGEP(
        i64Ty, newBuf,
        builder->CreateAdd(nEntryOff, llvm::ConstantInt::get(i64Ty, 1), "nkeyoff", /*HasNUW=*/true, /*HasNSW=*/true),
        "nkeyptr");
    {
        auto* st = builder->CreateAlignedStore(keyArg, nKeyPtr, llvm::MaybeAlign(8));
        st->setMetadata(llvm::LLVMContext::MD_tbaa, tbaaMapKey_);
    }
    llvm::Value* nValPtr = builder->CreateInBoundsGEP(
        i64Ty, newBuf,
        builder->CreateAdd(nEntryOff, llvm::ConstantInt::get(i64Ty, 2), "nvaloff", /*HasNUW=*/true, /*HasNSW=*/true),
        "nvalptr");
    {
...
llvm::Function* CodeGenerator::getOrEmitHashMapGet() {
    ...
    capLoad->setMetadata(llvm::LLVMContext::MD_invariant_load, llvm::MDNode::get(*context, {}));
    capLoad->setMetadata(llvm::LLVMContext::MD_tbaa, tbaaMapMeta_);
    llvm::Value* cap = capLoad;
...
llvm::Function* CodeGenerator::getOrEmitHashMapHas() {
    ...
    capLoad_has->setMetadata(llvm::LLVMContext::MD_invariant_load, llvm::MDNode::get(*context, {}));
    ...
    // Capacity is always a power of 2 >= 8.
    {
        llvm::Metadata* rangeMD[] = {llvm::ConstantAsMetadata::get(llvm::ConstantInt::get(i64Ty, 8)),
                                     llvm::ConstantAsMetadata::get(llvm::ConstantInt::get(i64Ty, 1ULL << 62))};
        capLoad_has->setMetadata(llvm::LLVMContext::MD_range, llvm::MDNode::get(*context, rangeMD));
        llvm::Value* capM1 = builder->CreateSub(cap, llvm::ConstantInt::get(i64Ty, 1));
        llvm::Value* isPow2 =
            builder->CreateICmpEQ(builder->CreateAnd(cap, capM1), llvm::ConstantInt::get(i64Ty, 0), "cap.pow2");
        llvm::Function* assumeFn = OMSC_GET_INTRINSIC(module.get(), llvm::Intrinsic::assume, {});
...
llvm::Function* CodeGenerator::getOrEmitHashMapRemove() {
    ...
    // Capacity is always a power of 2 >= 8.
    {
        llvm::Metadata* rangeMD[] = {llvm::ConstantAsMetadata::get(llvm::ConstantInt::get(i64Ty, 8)),
                                     llvm::ConstantAsMetadata::get(llvm::ConstantInt::get(i64Ty, 1ULL << 62))};
        capLoad_rm->setMetadata(llvm::LLVMContext::MD_range, llvm::MDNode::get(*context, rangeMD));
        llvm::Value* capM1 = builder->CreateSub(cap, llvm::ConstantInt::get(i64Ty, 1));
        llvm::Value* isPow2 =
            builder->CreateICmpEQ(builder->CreateAnd(cap, capM1), llvm::ConstantInt::get(i64Ty, 0), "cap.pow2");
        llvm::Function* assumeFn = OMSC_GET_INTRINSIC(module.get(), llvm::Intrinsic::assume, {});
    ...
    llvm::Value* newSize =
        builder->CreateSub(size, llvm::ConstantInt::get(i64Ty, 1), "newsize", /*HasNUW=*/true, /*HasNSW=*/true);
...
llvm::Function* CodeGenerator::getOrEmitHashMapKeys() {
    ...
    llvm::Value* arrSlots =
        builder->CreateAdd(size, llvm::ConstantInt::get(i64Ty, 1), "arrslots", /*HasNUW=*/true, /*HasNSW=*/true);
    ...
    builder->SetInsertPoint(bodyBB);
    llvm::Value* three = llvm::ConstantInt::get(i64Ty, 3);
    llvm::Value* off = builder->CreateAdd(builder->CreateMul(i, three, "i3", /*HasNUW=*/true, /*HasNSW=*/true),
                                          llvm::ConstantInt::get(i64Ty, 2), "off", /*HasNUW=*/true, /*HasNSW=*/true);
    ...
    llvm::Value* keyOff =
        builder->CreateAdd(off, llvm::ConstantInt::get(i64Ty, 1), "keyoff", /*HasNUW=*/true, /*HasNSW=*/true);
    ...
    llvm::Value* arrOff =
        builder->CreateAdd(writeIdx, llvm::ConstantInt::get(i64Ty, 1), "arroff", /*HasNUW=*/true, /*HasNSW=*/true);
    ...
    llvm::Value* newWIdx =
        builder->CreateAdd(writeIdx, llvm::ConstantInt::get(i64Ty, 1), "newidx", /*HasNUW=*/true, /*HasNSW=*/true);
    ...
    llvm::Value* iNext =
        builder->CreateAdd(i, llvm::ConstantInt::get(i64Ty, 1), "inext", /*HasNUW=*/true, /*HasNSW=*/true);
...
llvm::Function* CodeGenerator::getOrEmitHashMapValues() {
    ...
    builder->SetInsertPoint(storeBB);
    llvm::Value* valOff =
        builder->CreateAdd(off, llvm::ConstantInt::get(i64Ty, 2), "valoff", /*HasNUW=*/true, /*HasNSW=*/true);
...
llvm::Function* CodeGenerator::getOrEmitHashMapSize() {
    ...
    builder->SetInsertPoint(entryBB);
    llvm::Value* sizePtr =
        builder->CreateInBoundsGEP(i64Ty, fn->getArg(0), llvm::ConstantInt::get(i64Ty, 1), "sizeptr");
    ...
    // Size is always non-negative.
    {
        llvm::Metadata* rangeMD[] = {llvm::ConstantAsMetadata::get(llvm::ConstantInt::get(i64Ty, 0)),
                                     llvm::ConstantAsMetadata::get(llvm::ConstantInt::get(i64Ty, 1ULL << 62))};
        sizeLoad_sz->setMetadata(llvm::LLVMContext::MD_range, llvm::MDNode::get(*context, rangeMD));
    }
...
static bool
isPreAnalysisArrayExpr(Expression* expr, const llvm::StringSet<>& arrayReturningFunctions,
...
void CodeGenerator::preAnalyzeArrayTypes(Program* program) {
    ...
    auto scanStmt = [&](auto& self, Statement* s) -> bool {
        ...
        if (auto* ret = dynamic_cast<ReturnStmt*>(s)) {
            return ret->value &&
                   isPreAnalysisArrayExpr(ret->value.get(), arrayReturningFunctions_, funcParamArrayTypes_, nullptr);
        }
        if (auto* blk = dynamic_cast<BlockStmt*>(s)) {
            ...
            if (self(self, ifS->thenBranch.get()))
                return true;
            if (self(self, ifS->elseBranch.get()))
                return true;
        } else if (auto* wh = dynamic_cast<WhileStmt*>(s)) {
            if (self(self, wh->body.get()))
                return true;
        } else if (auto* dw = dynamic_cast<DoWhileStmt*>(s)) {
            if (self(self, dw->body.get()))
                return true;
        } else if (auto* forS = dynamic_cast<ForStmt*>(s)) {
            if (self(self, forS->body.get()))
                return true;
        } else if (auto* fe = dynamic_cast<ForEachStmt*>(s)) {
            if (self(self, fe->body.get()))
                return true;
        }
        return false;
    };
            ...
            const std::function<void(Statement*)> scanCallSites = [&](Statement* st) {
                ...
                auto scanExpr = [&](auto& se, Expression* e) -> void {
                    ...
                    if (e->type == ASTNodeType::CALL_EXPR) {
                        ...
                        for (size_t i = 0; i < call->arguments.size(); ++i) {
                            if (isPreAnalysisArrayExpr(call->arguments[i].get(), arrayReturningFunctions_,
                ...
                auto scanSt = [&](auto& ss, Statement* s) -> void {
                    ...
                    if (auto* es = dynamic_cast<ExprStmt*>(s))
                        ...
                        for (auto& st2 : blk->statements)
                            ss(ss, st2.get());
                    else if (auto* ifS = dynamic_cast<IfStmt*>(s)) {
                        ss(ss, ifS->thenBranch.get());
                        ss(ss, ifS->elseBranch.get());
                    } else if (auto* wh = dynamic_cast<WhileStmt*>(s))
                        ss(ss, wh->body.get());
                    else if (auto* dw = dynamic_cast<DoWhileStmt*>(s))
                        ss(ss, dw->body.get());
...
bool CodeGenerator::isStringExpr(Expression* expr) const {
    ...
    if (expr->type == ASTNodeType::CALL_EXPR) {
        ...
        if (arrayReturningFunctions_.count(call->callee))
            return false;
        return stringReturningFunctions_.count(call->callee) > 0;
    }
    return false;
...
void CodeGenerator::generate(Program* program) {
    ...
    for (auto& func : program->functions) {
        ...
        llvm::Function* function = llvm::Function::Create(funcType, linkage, func->name, module.get());
        ...
        function->addFnAttr("prefer-vector-width", std::to_string(preferredVectorWidth_ * 64));
        ...
        if (retType->isPointerTy() && optimizationLevel >= OptimizationLevel::O1) {
            ...
            function->addRetAttr(llvm::Attribute::getWithDereferenceableBytes(*context, 8));
        }
    ...
    if (verbose_) {
        std::cout << "  [codegen] Generating LLVM IR for " << program->functions.size() << " functions..." << '\n';
    }
        ...
        while (memEffChanged) {
            ...
            for (auto& func : module->functions()) {
                    ...
                    for (auto& I : BB) {
                        if (auto* SI = llvm::dyn_cast<llvm::StoreInst>(&I)) {
                            ...
                            if (!calledFn) {
                                ...
                                if (!ME.doesNotAccessMemory() && !calledFn->onlyAccessesArgMemory())
                                    allAccessesThroughArgs = false;
                            }
                        }
                    }
                ...
                if (!hasUnknownSideEffect && !hasMemoryWrite && !hasMemoryRead) {
                    ...
                    func.addFnAttr(llvm::Attribute::getWithMemoryEffects(*context, llvm::MemoryEffects::none()));
                    memEffChanged = true;
                    ...
                    if (!hasMemoryWrite) {
                        ...
                        func.addFnAttr(
                            llvm::Attribute::getWithMemoryEffects(*context, llvm::MemoryEffects::argMemOnly()));
                    }
                    memEffChanged = true;
                } else if (!hasUnknownSideEffect && !hasMemoryWrite) {
                    // Function reads memory (possibly non-arg) → readonly.
                    func.addFnAttr(llvm::Attribute::getWithMemoryEffects(*context, llvm::MemoryEffects::readOnly()));
                    memEffChanged = true;
                }
    ...
    if (optimizationLevel >= OptimizationLevel::O2) {
        for (auto& gv : module->globals()) {
            if (!gv.isConstant())
                continue;
            if (gv.getUnnamedAddr() != llvm::GlobalValue::UnnamedAddr::None)
                continue;
...
llvm::Function* CodeGenerator::generateFunction(FunctionDecl* func) {
    ...
    if (func->hintConstEval) {
        ...
        if (optCtx_)
            optCtx_->mutableFacts(func->name).isConstFoldable = true;
        function->addFnAttr(llvm::Attribute::InlineHint);
    ...
    if (!func->hintPure && optCtx_) {
        ...
        if (fx.willReturn() && !isSelfRecursive)
            function->setWillReturn();
        if (!fx.isReadNone() && !fx.isReadOnly() && fx.argMemOnly() && !func->hintRestrict && !fileNoAlias_) {
            function->setOnlyAccessesArgMemory();
        }
        // Per-parameter readonly: tag pointer arguments that the analysis
        if (!fx.paramMutated.empty()) {
            const std::size_t n = std::min<std::size_t>(fx.paramMutated.size(), function->arg_size());
            for (std::size_t i = 0; i < n; ++i) {
                if (fx.paramMutated[i])
                    continue;
    ...
    if (func->hintMemoryEffect != FunctionDecl::MemoryEffect::Default &&
        ...
        case ME::None:
            function->addFnAttr(llvm::Attribute::getWithMemoryEffects(*context, llvm::MemoryEffects::none()));
        ...
        case ME::ReadOnly:
            function->addFnAttr(
                llvm::Attribute::getWithMemoryEffects(*context, llvm::MemoryEffects(llvm::ModRefInfo::Ref)));
            function->setNoSync();
            if (!isSelfRecursive)
                function->setWillReturn();
            break;
        case ME::WriteOnly:
            function->addFnAttr(
                llvm::Attribute::getWithMemoryEffects(*context, llvm::MemoryEffects(llvm::ModRefInfo::Mod)));
            break;
        case ME::ArgMem:
            function->addFnAttr(llvm::Attribute::getWithMemoryEffects(*context, llvm::MemoryEffects::argMemOnly()));
            break;
        case ME::ArgMemRO:
        ...
        case ME::InaccessibleMem:
            function->addFnAttr(
                llvm::Attribute::getWithMemoryEffects(*context, llvm::MemoryEffects::inaccessibleMemOnly()));
            break;
        case ME::InaccessibleOrArgMem:
            function->addFnAttr(
                llvm::Attribute::getWithMemoryEffects(*context, llvm::MemoryEffects::inaccessibleOrArgMemOnly()));
            break;
        default:
            break;
        }
    ...
    if (optimizationLevel >= OptimizationLevel::O2 && !function->hasFnAttribute(llvm::Attribute::NoUnwind)) {
        function->addFnAttr(llvm::Attribute::NoUnwind);
    }
    ...
    if (optCtx_ && function->getReturnType()->isIntegerTy(64)) {
        if (auto rng = optCtx_->returnRange(func->name)) {
            if (rng->isNarrowed() && !rng->isEmpty() && rng->hi < std::numeric_limits<int64_t>::max()) {
                ...
                function->addRetAttr(
                    llvm::Attribute::get(*context, llvm::Attribute::Range, llvm::ConstantRange(apLo, apHi)));
            }
        }
    }
#endif
    ...
    for (size_t paramIdx = 0; paramIdx < func->parameters.size(); ++paramIdx) {
        ...
        llvm::AllocaInst* alloca = createEntryBlockAlloca(function, param.name, argIt->getType());
        {
            ...
            if (tbaaScalar_ &&
                (paramTy->isIntegerTy() || paramTy->isFloatTy() || paramTy->isDoubleTy() || paramTy->isPointerTy()))
        ...
        if (param.hintPrefetch) {
            ...
            llvm::Value* ptr =
                builder->CreateIntToPtr(ptrVal, llvm::PointerType::getUnqual(*context), param.name + ".pf.ptr");
            llvm::Function* prefetchFn =
                OMSC_GET_INTRINSIC(module.get(), llvm::Intrinsic::prefetch, {llvm::PointerType::getUnqual(*context)});
    ...
    if (inOptMaxFunction && !currentOptMaxConfig_.assumes.empty()) {
        ...
        for (const auto& assumeStr : currentOptMaxConfig_.assumes) {
            ...
            if (!(ss >> varName >> op >> litStr))
                continue;
            int64_t litVal = 0;
            try {
                litVal = std::stoll(litStr);
            } catch (...) {
                continue;
            }
            ...
            if (!alloca)
                continue;
            llvm::Value* loaded = builder->CreateLoad(alloca->getAllocatedType(), alloca, varName + ".assume.load");
            ...
            if (op == ">")
                cond = builder->CreateICmpSGT(cmpVal, lit, varName + ".assume.cond");
            else if (op == ">=")
                cond = builder->CreateICmpSGE(cmpVal, lit, varName + ".assume.cond");
            else if (op == "<")
                cond = builder->CreateICmpSLT(cmpVal, lit, varName + ".assume.cond");
            else if (op == "<=")
                cond = builder->CreateICmpSLE(cmpVal, lit, varName + ".assume.cond");
            else if (op == "!=")
                cond = builder->CreateICmpNE(cmpVal, lit, varName + ".assume.cond");
            else if (op == "==")
                cond = builder->CreateICmpEQ(cmpVal, lit, varName + ".assume.cond");
            if (cond) {
    ...
    if (inOptMaxFunction && currentOptMaxConfig_.memory.prefetch) {
        ...
        for (size_t i = 0; i < func->parameters.size(); ++i) {
            ...
            if (prefetchedParams_.count(param.name))
            ...
            if (!al)
                continue;
            llvm::Value* val = builder->CreateLoad(al->getAllocatedType(), al, param.name + ".autopf.load");
            // Treat as a pointer (i64 → ptr cast) and prefetch the target memory.
            llvm::Value* ptr =
                val->getType()->isPointerTy()
                    ? val
                    : builder->CreateIntToPtr(val, llvm::PointerType::getUnqual(*context), param.name + ".autopf.ptr");
            builder->CreateCall(prefetchFn, {
                                                ptr,
                                                builder->getInt32(0), // read prefetch
    ...
    if (!builder->GetInsertBlock()->getTerminator()) {
        ...
        for (const auto& pfParam : prefetchedParams_) {
            ...
            if (it != namedValues.end()) {
                ...
                llvm::Function* prefetchFn = OMSC_GET_INTRINSIC(module.get(), llvm::Intrinsic::prefetch,
...
std::optional<int64_t> CodeGenerator::tryConstEval(const FunctionDecl* func, const std::vector<int64_t>& argVals) {
    ...
    evalExpr = [&](Expression* e) -> std::optional<int64_t> {
        ...
        case ASTNodeType::IDENTIFIER_EXPR: {
            ...
            if (eit != enumConstants_.end())
                return static_cast<int64_t>(eit->second);
            return std::nullopt;
        }
        case ASTNodeType::BINARY_EXPR: {
            auto* bin = static_cast<BinaryExpr*>(e);
            ...
            if (bin->op == "**") {
                if (b < 0)
                    return (a == 1) ? int64_t(1) : (a == -1 ? (b & 1 ? int64_t(-1) : int64_t(1)) : int64_t(0));
        ...
        case ASTNodeType::TERNARY_EXPR: {
            ...
            if (!cond)
                return std::nullopt;
            return *cond != 0 ? evalExpr(tern->thenExpr.get()) : evalExpr(tern->elseExpr.get());
        }
        case ASTNodeType::CALL_EXPR: {
            auto* call = static_cast<CallExpr*>(e);
...
std::optional<CodeGenerator::ConstValue> CodeGenerator::tryFoldExprToConst(Expression* expr, int depth) const {
    (void)depth;
    if (!expr || !ctEngine_)
        return std::nullopt;
    const auto env = buildComptimeEnv();
    const CTValue result = ctEngine_->evalSingleExpr(env, expr);
    if (!result.isKnown() || result.isSymbolic())
        return std::nullopt;
...
std::optional<std::string> CodeGenerator::tryFoldStr(Expression* e) const {
    if (!e)
        return std::nullopt;
    if (auto cv = tryFoldExprToConst(e))
        if (cv->kind == ConstValue::Kind::String)
            return cv->strVal;
    return std::nullopt;
}
...
CodeGenerator::tryConstEvalFull(const FunctionDecl* func, const std::unordered_map<std::string, ConstValue>& argEnv,
                                int depth) const {
    (void)depth;
    if (!func || !func->body || !ctEngine_)
        return std::nullopt;
    ...
    if (!result || !result->isKnown() || result->isSymbolic())
        return std::nullopt;
    return ctValueToConstValue(*result);
}
...
std::optional<CodeGenerator::ConstValue>
CodeGenerator::tryConstEvalFull(const BlockStmt* body, const std::unordered_map<std::string, ConstValue>& argEnv,
                                int depth) const {
    (void)depth;
    if (!body || !ctEngine_)
        return std::nullopt;
...
void CodeGenerator::analyzeConstantReturnValues(Program* program) {
    ...
    for (auto& func : program->functions) {
        ...
        if (!func->body || !func->parameters.empty())
            continue;
        if (optCtx_ && (optCtx_->constIntReturn(fname) || optCtx_->constStringReturn(fname)))
            continue;
        if (!ctEngine_->isPure(fname))
            continue;
        ...
        if (result->isInt()) {
            if (optCtx_)
                optCtx_->mutableFacts(fname).constIntReturn = result->asI64();
        } else if (result->isString()) {
            if (optCtx_)
                optCtx_->mutableFacts(fname).constStringReturn = result->asStr();
        }
    }
}
...
void CodeGenerator::runCFCTRE(Program* program) {
    ...
    for (auto& fn : program->functions) {
        if (!fn->parameters.empty())
            continue;
        if (!ctEngine_->isPure(fn->name))
            continue;
        // Query the memoised zero-arg result.
        auto result = ctEngine_->executeFunction(fn->name, {});
        if (!result)
            continue;
        if (result->isInt() && !(optCtx_ && optCtx_->constIntReturn(fn->name))) {
            if (optCtx_)
                optCtx_->mutableFacts(fn->name).constIntReturn = result->asI64();
        } else if (result->isString() && !(optCtx_ && optCtx_->constStringReturn(fn->name))) {
            if (optCtx_)
                optCtx_->mutableFacts(fn->name).constStringReturn = result->asStr();
        }
    }
    ...
    for (auto& [name, ctVal] : ctEngine_->uniformReturnValues()) {
        if (ctVal.isInt() && !(optCtx_ && optCtx_->constIntReturn(name))) {
            if (optCtx_)
                optCtx_->mutableFacts(name).constIntReturn = ctVal.asI64();
        } else if (ctVal.isString() && !(optCtx_ && optCtx_->constStringReturn(name))) {
            if (optCtx_)
                optCtx_->mutableFacts(name).constStringReturn = ctVal.asStr();
        }
    }
    ...
    if (!ctEngine_->deadFunctions().empty()) {
        ...
        for (auto& fn : program->functions) {
            if (!dead.count(fn->name))
                continue;
            if (fn->hintCold)
                continue; // user already annotated
            fn->hintCold = true;
    ...
    if (verbose_) {
        const auto& s = ctEngine_->stats();
        std::cout << "  [cfctre] Pass complete: " << s.functionsRegistered << " functions registered, "
                  << s.pureFunctionsDetected << " pure, " << s.functionCallsMemoized << " calls memoised, "
                  << s.arraysAllocated << " arrays allocated, " << s.loopsReasoned << " loops reasoned, "
                  << s.branchMerges << " branch merges, " << s.ternaryMerges << " ternary merges, "
                  << s.uniformReturnFunctionsFound << " uniform-return, " << s.deadFunctionsDetected << " dead\n"
                  << "  [cfctre/absinterp] " << s.deadBranchesEliminated << " dead branches, " << s.safeArrayAccesses
                  << " safe array accesses, " << s.safeDivisions << " safe divisions, " << s.safeArithmetic
                  << " safe arithmetic ops, " << s.cheaperRewritesFound << " cheaper rewrites" << '\n';
    }
    ...
    if (ctEngine_ && optimizationLevel >= OptimizationLevel::O2) {
        ...
        for (auto& fn : program->functions) {
            if (fn->hintNoInline)
                continue;
            if (fn->parameters.empty())
                continue; // zero-arg already folded
            if (!foldable.count(fn->name))
                continue;
...
void CodeGenerator::runEGraphPass(Program* program, OptimizationContext& ctx) {
    ...
    if (verbose_) {
        std::cout << "  [opt] Running e-graph equality saturation on AST (" << program->functions.size()
                  << " functions, maxNodes=" << cfg.maxNodes << ", maxIter=" << cfg.maxIterations << ")..." << '\n';
    }
    ...
    if (verbose_) {
        ...
        std::cout << "  [opt] E-graph saturation complete: " << s.expressionsSimplified << "/" << s.expressionsAttempted
                  << " expressions simplified, " << s.functionsChanged << " functions changed, " << s.expressionsSkipped
...
CTValue CodeGenerator::constValueToCTValue(const ConstValue& v) const {
    ...
    case ConstValue::Kind::Integer:
        return CTValue::fromI64(v.intVal);
    case ConstValue::Kind::Float:
        return CTValue::fromF64(v.floatVal);
    case ConstValue::Kind::String:
        return CTValue::fromString(v.strVal);
    case ConstValue::Kind::Array: {
        ...
        for (size_t i = 0; i < v.arrVal.size(); ++i)
            hp.store(h, static_cast<int64_t>(i), constValueToCTValue(v.arrVal[i]));
...
std::unordered_map<std::string, CTValue> CodeGenerator::buildComptimeEnv() const {
...
void CodeGenerator::autoDetectConstEvalFunctions(Program* program) {
    if (!program || optimizationLevel < OptimizationLevel::O1)
        return;
    ...
    isStmtPure = [&](const Statement* stmt) -> bool {
        ...
        case ASTNodeType::IF_STMT: {
            ...
            return isExprPure(ifS->condition.get()) && isStmtPure(ifS->thenBranch.get()) &&
        ...
        case ASTNodeType::FOR_STMT: {
            ...
            return isExprPure(fs->start.get()) && isExprPure(fs->end.get()) &&
                   (!fs->step || isExprPure(fs->step.get())) && isStmtPure(fs->body.get());
        }
        case ASTNodeType::EXPR_STMT: {
        ...
        case ASTNodeType::SWITCH_STMT: {
            ...
            if (!isExprPure(sw->condition.get()))
                return false;
            for (const auto& c : sw->cases) {
                if (c.value && !isExprPure(c.value.get()))
                    return false;
    ...
    while (changed) {
        ...
        for (const auto& func : program->functions) {
            ...
            if (bodyIsPure) {
                ...
                if (optCtx_)
                    optCtx_->mutableFacts(fname).isConstFoldable = true;
                changed = true;
            }
        }
    }
...
void CodeGenerator::inferFunctionEffects(Program* program) {
    ...
    auto markNameStore = [&](const std::string& name, const FunctionDecl* self, FunctionEffects& fx) {
        if (!self)
            return;
    ...
    auto markNameLoad = [&](const std::string& name, const FunctionDecl*, FunctionEffects& fx) {
        if (globalNames.count(name))
            fx.readsGlobal = true;
    };
    ...
    exprEffects = [&](const Expression* expr, const FunctionDecl* self) -> FunctionEffects {
        ...
        case ASTNodeType::CALL_EXPR: {
            ...
            const bool isKnownBuiltin = bt.constFoldable || bt.readsMemory || bt.writesMemory || bt.hasIO ||
            ...
            if (isKnownBuiltin) {
                ...
                if (it != funcEffects.end()) {
                    ...
                    if (self) {
                        std::size_t n = std::min(cf.paramMutated.size(), call->arguments.size());
                        for (std::size_t i = 0; i < n; ++i) {
                            if (!cf.paramMutated[i])
                                continue;
        ...
        case ASTNodeType::BINARY_EXPR: {
            ...
            if (b->op == "/" || b->op == "%") {
                ...
                bool divisorIsNonZeroConst =
                    lit && lit->literalType == LiteralExpr::LiteralType::INTEGER && lit->intValue != 0;
        ...
        case ASTNodeType::POSTFIX_EXPR: {
            ...
            if (!n.empty())
                markNameStore(n, self, fx);
            else
                fx.mergeFrom(exprEffects(p->operand.get(), self));
            break;
        }
        case ASTNodeType::PREFIX_EXPR: {
    ...
    stmtEffects = [&](const Statement* stmt, const FunctionDecl* self) -> FunctionEffects {
        ...
        case ASTNodeType::VAR_DECL: {
            auto* vd = static_cast<const VarDecl*>(stmt);
            if (vd->initializer)
                fx.mergeFrom(exprEffects(vd->initializer.get(), self));
            break;
        }
        case ASTNodeType::MOVE_DECL: {
            auto* md = static_cast<const MoveDecl*>(stmt);
            if (md->initializer)
                fx.mergeFrom(exprEffects(md->initializer.get(), self));
            break;
        }
        case ASTNodeType::RETURN_STMT: {
            auto* ret = static_cast<const ReturnStmt*>(stmt);
            if (ret->value)
                fx.mergeFrom(exprEffects(ret->value.get(), self));
            break;
        }
        case ASTNodeType::BLOCK: {
        ...
        case ASTNodeType::FOR_STMT: {
            auto* fs = static_cast<const ForStmt*>(stmt);
            if (fs->start)
                fx.mergeFrom(exprEffects(fs->start.get(), self));
            if (fs->end)
                fx.mergeFrom(exprEffects(fs->end.get(), self));
            if (fs->step)
                fx.mergeFrom(exprEffects(fs->step.get(), self));
        ...
        case ASTNodeType::SWITCH_STMT: {
            ...
            for (const auto& c : sw->cases) {
                if (c.value)
                    fx.mergeFrom(exprEffects(c.value.get(), self));
                for (const auto& s : c.body)
                    fx.mergeFrom(stmtEffects(s.get(), self));
            }
            break;
        ...
        case ASTNodeType::THROW_STMT: {
            ...
            if (th->value)
                fx.mergeFrom(exprEffects(th->value.get(), self));
        ...
        case ASTNodeType::PIPELINE_STMT: {
            ...
            if (pl->count)
                fx.mergeFrom(exprEffects(pl->count.get(), self));
    ...
    const std::size_t kMaxIters = std::max<std::size_t>(8, program->functions.size() * 16);
    bool changed = true;
    std::size_t iter = 0;
...
llvm::GlobalVariable* CodeGenerator::internString(const std::string& content) {
    ...
    auto* globalInit =
        llvm::ConstantStruct::get(structTy, {llvm::ConstantInt::get(i64Ty, slen), // len
                                             llvm::ConstantInt::get(i64Ty, slen), // cap == len for literals
                                             charArr});
    auto* gv =
        new llvm::GlobalVariable(*module, structTy,
                                 /*isConstant=*/true, llvm::GlobalValue::PrivateLinkage, globalInit, ".str.intern");
    gv->setUnnamedAddr(llvm::GlobalValue::UnnamedAddr::Global);
...
static bool exprUsesVar(Expression* expr, const std::string& varName) {
    ...
    if (expr->type == ASTNodeType::CALL_EXPR) {
        auto* call = static_cast<CallExpr*>(expr);
        for (auto& arg : call->arguments) {
            if (exprUsesVar(arg.get(), varName))
                return true;
        }
    ...
    if (expr->type == ASTNodeType::BINARY_EXPR) {
        auto* bin = static_cast<BinaryExpr*>(expr);
        return exprUsesVar(bin->left.get(), varName) || exprUsesVar(bin->right.get(), varName);
    }
    ...
    if (expr->type == ASTNodeType::INDEX_EXPR) {
        ...
        return exprUsesVar(idx->array.get(), varName) || exprUsesVar(idx->index.get(), varName);
    }
...
static bool varEscapesInBlock(const BlockStmt* block, const std::string& varName, size_t startIdx);
...
static bool varEscapesInStmt(const Statement* s, const std::string& varName) {
    ...
    if (auto* es = dynamic_cast<const ExprStmt*>(s)) {
        if (exprUsesVar(es->expression.get(), varName)) {
            ...
            if (es->expression->type == ASTNodeType::ASSIGN_EXPR) {
                ...
                if (exprUsesVar(asgn->value.get(), varName))
                    return true;
            }
        }
    }
    ...
    if (auto* ifs = dynamic_cast<const IfStmt*>(s)) {
        return varEscapesInStmt(ifs->thenBranch.get(), varName) || varEscapesInStmt(ifs->elseBranch.get(), varName);
    }
    if (auto* ws = dynamic_cast<const WhileStmt*>(s)) {
        return exprUsesVar(ws->condition.get(), varName) || varEscapesInStmt(ws->body.get(), varName);
    }
    if (auto* dws = dynamic_cast<const DoWhileStmt*>(s)) {
        return exprUsesVar(dws->condition.get(), varName) || varEscapesInStmt(dws->body.get(), varName);
    }
...
static bool varEscapesInBlock(const BlockStmt* block, const std::string& varName, size_t startIdx) {
    for (size_t i = startIdx; i < block->statements.size(); ++i) {
        if (varEscapesInStmt(block->statements[i].get(), varName))
            return true;
    }
    return false;
}
...
bool CodeGenerator::doesVarEscapeCurrentScope(const std::string& varName) const {
    if (!currentFuncDecl_ || !currentFuncDecl_->body)
    ...
    std::function<std::optional<std::pair<const BlockStmt*, size_t>>(const BlockStmt*)> findDecl =
        [&](const BlockStmt* blk) -> std::optional<std::pair<const BlockStmt*, size_t>> {
    ...
    if (auto found = findDecl(body)) {
        ...
        if (varEscapesInBlock(declBlock, varName, declIdx + 1))
            return true;
...
static bool exprHasIndexAssignToVar(const Expression* expr, const std::string& varName) {
    ...
    case ASTNodeType::INDEX_ASSIGN_EXPR: {
        ...
        return exprHasIndexAssignToVar(ia->array.get(), varName) || exprHasIndexAssignToVar(ia->index.get(), varName) ||
               exprHasIndexAssignToVar(ia->value.get(), varName);
    }
    case ASTNodeType::BINARY_EXPR: {
        auto* bin = static_cast<const BinaryExpr*>(expr);
        return exprHasIndexAssignToVar(bin->left.get(), varName) || exprHasIndexAssignToVar(bin->right.get(), varName);
    }
    case ASTNodeType::UNARY_EXPR:
        return exprHasIndexAssignToVar(static_cast<const UnaryExpr*>(expr)->operand.get(), varName);
    case ASTNodeType::PREFIX_EXPR:
        return exprHasIndexAssignToVar(static_cast<const PrefixExpr*>(expr)->operand.get(), varName);
    case ASTNodeType::POSTFIX_EXPR:
        return exprHasIndexAssignToVar(static_cast<const PostfixExpr*>(expr)->operand.get(), varName);
    case ASTNodeType::TERNARY_EXPR: {
        auto* t = static_cast<const TernaryExpr*>(expr);
    ...
    case ASTNodeType::CALL_EXPR: {
        auto* call = static_cast<const CallExpr*>(expr);
        for (const auto& arg : call->arguments) {
            if (exprHasIndexAssignToVar(arg.get(), varName))
                return true;
        }
        return false;
    }
    case ASTNodeType::INDEX_EXPR: {
        ...
        return exprHasIndexAssignToVar(idx->array.get(), varName) || exprHasIndexAssignToVar(idx->index.get(), varName);
    }
    case ASTNodeType::ASSIGN_EXPR: {
    ...
    case ASTNodeType::ARRAY_EXPR: {
        auto* arr = static_cast<const ArrayExpr*>(expr);
        for (const auto& e : arr->elements) {
            if (exprHasIndexAssignToVar(e.get(), varName))
                return true;
        }
        return false;
    }
...
static bool stmtHasIndexAssignToVar(const Statement* s, const std::string& varName) {
    ...
    if (auto* blk = dynamic_cast<const BlockStmt*>(s)) {
        for (const auto& st : blk->statements)
            if (stmtHasIndexAssignToVar(st.get(), varName))
                return true;
        return false;
    }
    if (auto* ifs = dynamic_cast<const IfStmt*>(s))
    ...
    if (auto* fs = dynamic_cast<const ForStmt*>(s))
        return exprHasIndexAssignToVar(fs->start.get(), varName) || exprHasIndexAssignToVar(fs->end.get(), varName) ||
...
struct ReadOnlyUseChecker {
    ...
    explicit ReadOnlyUseChecker(const std::string& v, const CTEngine* eng) : var(v), ctEngine(eng) {}

    bool isVarIdent(const Expression* e) const {
        return e && e->type == ASTNodeType::IDENTIFIER_EXPR && static_cast<const IdentifierExpr*>(e)->name == var;
    }
    ...
    bool isCallNonMutating(const std::string& callee) const {
        ...
        if (BuiltinEffectTable::isPure(callee))
            return true;
        if (BuiltinEffectTable::isReadOnly(callee))
            return true;
        if (BuiltinEffectTable::isMutating(callee))
            return false;
        // I/O builtins (print, etc.) read their args without mutating.
        if (ctEngine && ctEngine->isPure(callee))
            return true;
        return false;
    }
...
static bool exprEqual(const Expression* a, const Expression* b) {
    ...
    if (a->type == ASTNodeType::LITERAL_EXPR) {
        ...
        if (la->literalType != lb->literalType)
            return false;
...
void CodeGenerator::fuseLoops(BlockStmt* block) {
    ...
    while (i + 1 < stmts.size()) {
        ...
        if (!exprEqual(first->start.get(), second->start.get()) || !exprEqual(first->end.get(), second->end.get())) {
            ++i;
            continue;
        }
        ...
        if (second->iteratorVar != first->iteratorVar) {
            ...
            auto aliasDecl = std::make_unique<VarDecl>(second->iteratorVar, std::move(aliasIdent), false, "");
        ...
        auto fused = std::make_unique<ForStmt>(first->iteratorVar, std::move(first->start), std::move(first->end),
                                               std::move(first->step), std::move(fusedBody), first->iteratorType);
...
void CodeGenerator::generateGlobals(Program* program) {
    ...
    for (const auto& gv : program->globals) {
        ...
        llvm::Type* ty = gv->typeName.empty() ? getDefaultType() : resolveAnnotatedType(gv->typeName);
        ...
        auto* llvmGV = new llvm::GlobalVariable(*module, ty,
