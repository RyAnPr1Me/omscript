...
uint64_t hashProfile(const BuildProfile& p) {
    ...
    ss << static_cast<int>(p.optLevel) << static_cast<int>(p.egraph) << static_cast<int>(p.superopt) << p.superoptLevel
       << static_cast<int>(p.debugInfo) << static_cast<int>(p.strip) << static_cast<int>(p.lto)
       << static_cast<int>(p.fastMath) << static_cast<int>(p.optMax) << static_cast<int>(p.vectorize)
       << static_cast<int>(p.unrollLoops) << static_cast<int>(p.loopOptimize) << static_cast<int>(p.parallelize)
       << static_cast<int>(p.hgoe) << static_cast<int>(p.wholeProgram) << p.outDir;
...
static std::vector<std::string> scanImports(const std::string& source) {
    ...
    while ((pos = source.find(needle, pos)) != std::string::npos) {
        ...
        if (pos > 0) {
            const unsigned char prev = static_cast<unsigned char>(source[pos - 1]);
        ...
        if (after < source.size()) {
            const unsigned char next = static_cast<unsigned char>(source[after]);
...
void BuildGraph::scanFile(const std::string& absPath, const std::string& projectDir) {
    namespace fs = std::filesystem;

    // Guard against revisiting (handles circular imports safely).
    if (nodes_.count(absPath))
        return;
...
void BuildGraph::load(const std::string& entryPath, const std::string& projectDir) {
    namespace fs = std::filesystem;
    nodes_.clear();
    entryPath_.clear();
...
std::string BuildGraph::computeFingerprint(uint64_t manifestHash, uint64_t profileHash) const {
...
BuildCache::BuildCache(std::string cacheDir) : cacheDir_(std::move(cacheDir)) {
