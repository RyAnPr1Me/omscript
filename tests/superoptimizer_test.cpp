...
// ─────────────────────────────────────────────────────────────────────────────
// New synthesis tests for extended multiply-by-constant patterns
// ─────────────────────────────────────────────────────────────────────────────

TEST(SuperoptimizerTest, SynthGeneralPow2Plus1) {
    // x * 129 = x * (2^7 + 1) → (x << 7) + x
    TestModule tm;
    auto* a = tm.arg(0);
    auto* c = llvm::ConstantInt::get(tm.i64Ty(), 129);

    auto* mul = tm.builder.CreateMul(a, c, "mul129");
    tm.builder.CreateRet(mul);
...
TEST(SuperoptimizerTest, SynthGeneralPow2Minus1) {
    // x * 511 = x * (2^9 - 1) → (x << 9) - x
    TestModule tm;
    auto* a = tm.arg(0);
    auto* c = llvm::ConstantInt::get(tm.i64Ty(), 511);

    auto* mul = tm.builder.CreateMul(a, c, "mul511");
    tm.builder.CreateRet(mul);
...
TEST(SuperoptimizerTest, SynthMul1023) {
    // x * 1023 = x * (2^10 - 1) → (x << 10) - x
    TestModule tm;
    auto* a = tm.arg(0);
    auto* c = llvm::ConstantInt::get(tm.i64Ty(), 1023);

    auto* mul = tm.builder.CreateMul(a, c, "mul1023");
    tm.builder.CreateRet(mul);
...
TEST(SuperoptimizerTest, SynthMul257) {
    // x * 257 = x * (2^8 + 1) → (x << 8) + x
    TestModule tm;
    auto* a = tm.arg(0);
    auto* c = llvm::ConstantInt::get(tm.i64Ty(), 257);

    auto* mul = tm.builder.CreateMul(a, c, "mul257");
    tm.builder.CreateRet(mul);
...
TEST(SuperoptimizerTest, SynthMul1025) {
    // x * 1025 = x * (2^10 + 1) → (x << 10) + x
    TestModule tm;
    auto* a = tm.arg(0);
    auto* c = llvm::ConstantInt::get(tm.i64Ty(), 1025);

    auto* mul = tm.builder.CreateMul(a, c, "mul1025");
    tm.builder.CreateRet(mul);
...
TEST(SuperoptimizerTest, AlgebraicAndAllOnes) {
    TestModule tm;
    auto* a = tm.arg(0);
    auto* allOnes = llvm::ConstantInt::get(tm.i64Ty(), -1);

    auto* andI = tm.builder.CreateAnd(a, allOnes, "and_allones");
    tm.builder.CreateRet(andI);
...
TEST(SuperoptimizerTest, AlgebraicOrAllOnes) {
    TestModule tm;
    auto* a = tm.arg(0);
    auto* allOnes = llvm::ConstantInt::get(tm.i64Ty(), -1);

    auto* orI = tm.builder.CreateOr(a, allOnes, "or_allones");
    tm.builder.CreateRet(orI);
...
TEST(SuperoptimizerTest, AlgebraicMul2ToShift) {
    TestModule tm;
    auto* a = tm.arg(0);
    auto* two = llvm::ConstantInt::get(tm.i64Ty(), 2);

    auto* mul = tm.builder.CreateMul(a, two, "mul_two");
    tm.builder.CreateRet(mul);
...
// ─────────────────────────────────────────────────────────────────────────────
// Integration test: compile C-like function with optimizations
// ─────────────────────────────────────────────────────────────────────────────

TEST(SuperoptimizerTest, CompileCLikeSumArrayFunction) {
...
TEST(SuperoptimizerTest, CompileCLikeMultiplyOptFunction) {
    // Test superoptimizer on C-like multiply patterns:
    //   int compute(int x) {
    //       return x * 3 + x * 7 + x * 15;
    //   }
    // The superoptimizer should convert multiplies to shift+add/sub
    CodeGenerator codegen(OptimizationLevel::O2);
    auto* mod = compileOmScript(
        "fn compute(x) {\n"
        "  return x * 3 + x * 7 + x * 15;\n"
        "}\n"
        "fn main() { return compute(10); }",
        codegen);
    ASSERT_NE(mod, nullptr);
...
TEST(SuperoptimizerTest, CompileCLikeMinMaxFunction) {
    // Test superoptimizer idiom detection on min/max patterns:
...
// ─────────────────────────────────────────────────────────────────────────────
// New multiply strength reduction cases
// ─────────────────────────────────────────────────────────────────────────────

TEST(SuperoptimizerTest, StrengthReductionMul30) {
    // x * 30 → (x << 5) - (x << 1)  [32x - 2x = 30x]
    TestModule tm;
    auto* x = tm.arg(0);
    auto* mul = tm.builder.CreateMul(x, llvm::ConstantInt::get(tm.i64Ty(), 30), "mul30");
    tm.builder.CreateRet(mul);
...
TEST(SuperoptimizerTest, StrengthReductionMul34) {
    // x * 34 → (x << 5) + (x << 1)  [32x + 2x = 34x]
    TestModule tm;
    auto* x = tm.arg(0);
    auto* mul = tm.builder.CreateMul(x, llvm::ConstantInt::get(tm.i64Ty(), 34), "mul34");
    tm.builder.CreateRet(mul);
...
TEST(SuperoptimizerTest, StrengthReductionMul36) {
    // x * 36 → (x << 5) + (x << 2)  [32x + 4x = 36x]
    TestModule tm;
    auto* x = tm.arg(0);
    auto* mul = tm.builder.CreateMul(x, llvm::ConstantInt::get(tm.i64Ty(), 36), "mul36");
    tm.builder.CreateRet(mul);
...
TEST(SuperoptimizerTest, StrengthReductionMul56) {
    // x * 56 → (x << 6) - (x << 3)  [64x - 8x = 56x]
    TestModule tm;
    auto* x = tm.arg(0);
    auto* mul = tm.builder.CreateMul(x, llvm::ConstantInt::get(tm.i64Ty(), 56), "mul56");
    tm.builder.CreateRet(mul);
...
TEST(SuperoptimizerTest, StrengthReductionMul60) {
    // x * 60 → (x << 6) - (x << 2)  [64x - 4x = 60x]
    TestModule tm;
    auto* x = tm.arg(0);
    auto* mul = tm.builder.CreateMul(x, llvm::ConstantInt::get(tm.i64Ty(), 60), "mul60");
    tm.builder.CreateRet(mul);
...
TEST(SuperoptimizerTest, StrengthReductionMul72) {
    // x * 72 → (x << 6) + (x << 3)  [64x + 8x = 72x]
    TestModule tm;
    auto* x = tm.arg(0);
    auto* mul = tm.builder.CreateMul(x, llvm::ConstantInt::get(tm.i64Ty(), 72), "mul72");
    tm.builder.CreateRet(mul);
...
TEST(SuperoptimizerTest, StrengthReductionMul80) {
    // x * 80 → (x << 6) + (x << 4)  [64x + 16x = 80x]
    TestModule tm;
    auto* x = tm.arg(0);
    auto* mul = tm.builder.CreateMul(x, llvm::ConstantInt::get(tm.i64Ty(), 80), "mul80");
    tm.builder.CreateRet(mul);
...
TEST(SuperoptimizerTest, StrengthReductionMul192) {
    // x * 192 → (x << 7) + (x << 6)  [128x + 64x = 192x]
    TestModule tm;
    auto* x = tm.arg(0);
    auto* mul = tm.builder.CreateMul(x, llvm::ConstantInt::get(tm.i64Ty(), 192), "mul192");
    tm.builder.CreateRet(mul);
...
TEST(SuperoptimizerTest, OrSubAndToXor) {
    // (a | b) - (a & b) → a ^ b
    TestModule tm;
    auto* a = tm.arg(0);
    auto* b = tm.arg(1);
    auto* orVal  = tm.builder.CreateOr(a, b, "or_val");
    auto* andVal = tm.builder.CreateAnd(a, b, "and_val");
    auto* sub    = tm.builder.CreateSub(orVal, andVal, "or_sub_and");
    tm.builder.CreateRet(sub);
