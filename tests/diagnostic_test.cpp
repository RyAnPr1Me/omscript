...
TEST(DiagnosticTest, FormatWithFilename) {
    Diagnostic diag;
    diag.severity = DiagnosticSeverity::Error;
    diag.location = {"main.om", 10, 5};
    diag.message = "undefined variable 'x'";
    EXPECT_EQ(diag.format(), "main.om:10:5: error: undefined variable 'x'");
}

TEST(DiagnosticTest, FormatWithFilenameWarning) {
    Diagnostic diag;
    diag.severity = DiagnosticSeverity::Warning;
    diag.location = {"lib/utils.om", 42, 1};
    diag.message = "unused import";
    EXPECT_EQ(diag.format(), "lib/utils.om:42:1: warning: unused import");
}
...
TEST(DiagnosticTest, DiagnosticErrorWithFilename) {
    Diagnostic diag;
    diag.severity = DiagnosticSeverity::Error;
    diag.location = {"test.om", 7, 12};
