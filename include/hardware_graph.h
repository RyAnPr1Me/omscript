...
inline const char* fpPrecisionName(FPPrecision p) {
    switch (p) {
    case FPPrecision::Strict:
        return "strict";
    case FPPrecision::Medium:
        return "medium";
    case FPPrecision::Fast:
        return "fast";
    }
    return "unknown";
}
...
class HardwareGraph {
  public:
    ...
    unsigned addNode(ResourceType type, const std::string& name, unsigned count = 1, double latency = 1.0,
                     double throughput = 1.0, unsigned pipelineDepth = 1);

    /// Add a directed edge between two hardware nodes.
    void addEdge(unsigned srcId, unsigned dstId, double latency = 0.0, double bandwidth = 1.0,
    ...
    [[nodiscard]] const std::vector<HardwareNode>& nodes() const noexcept {
        return nodes_;
    }

    /// Get all edges (read-only).
    [[nodiscard]] const std::vector<HardwareEdge>& edges() const noexcept {
        return edges_;
    }
...
class ProgramGraph {
  public:
    ...
    void addEdge(unsigned srcId, unsigned dstId, DepType type = DepType::Data, unsigned latency = 1);
    ...
    /// Get all nodes (read-only).
    const std::vector<ProgramNode>& nodes() const {
        return nodes_;
    }

    /// Get all edges (read-only).
    const std::vector<ProgramEdge>& edges() const {
        return edges_;
    }
...
class HardwareCostModel {
  public:
    ...
    unsigned preferredVectorWidth() const {
        return vectorWidth_;
    }
...
unsigned scheduleInstructions(llvm::Function& func, const HardwareGraph& hw, const MicroarchProfile& profile,
                              const SchedulerPolicy& policy = SchedulerPolicy{}, SchedulerQuality* quality = nullptr);
