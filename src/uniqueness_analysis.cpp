...
static bool isKnownFreshStringBuiltin(const std::string& callee) noexcept {
    ...
    static constexpr const char* kNames[] = {
        "format",    "input",        "str_concat",    "str_format", "str_join",    "str_lower",
        "str_ltrim", "str_pad_left", "str_pad_right", "str_repeat", "str_replace", "str_reverse",
        "str_rtrim", "str_slice",    "str_trim",      "str_upper",  "substr",      "to_string",
    };
...
static bool isKnownFreshArrayBuiltin(const std::string& callee) noexcept {
    static constexpr const char* kNames[] = {
        "array_concat", "array_copy", "array_fill", "array_filter", "array_map", "array_remove", "array_slice",
        "pop",          "push",       "reverse",    "sort",         "str_chars", "str_split",    "unshift",
    };
    static constexpr size_t kN = sizeof(kNames) / sizeof(kNames[0]);
    const char* const* lo = kNames;
    const char* const* hi = kNames + kN;
...
static bool isKnownNonCapturingBuiltin(const std::string& callee) noexcept {
    ...
    static constexpr const char* kNames[] = {
        "assert",    "contains",      "ends_with", "eprint",       "eprintln",        "find",
        "is_null",   "len",           "print",     "println",      "starts_with",     "str_contains",
        "str_count", "str_ends_with", "str_find",  "str_index_of", "str_starts_with", "type_of",
    };
    static constexpr size_t kN = sizeof(kNames) / sizeof(kNames[0]);
    const char* const* lo = kNames;
    const char* const* hi = kNames + kN;
...
static bool isFreshStringExpr(const Expression* expr, const std::unordered_set<std::string>& strVars,
                              const std::unordered_set<std::string>& arrVars) noexcept;

/// Returns true if @p expr evaluates to a freshly-allocated array buffer.
static bool isFreshArrayExpr(const Expression* expr, const std::unordered_set<std::string>& strVars,
                             const std::unordered_set<std::string>& arrVars) noexcept;
...
static bool isFreshStringExpr(const Expression* expr, const std::unordered_set<std::string>& strVars,
        ...
        if (bin->op == "+" || bin->op == "*") {
            ...
            const bool leftStr = isFreshStringExpr(bin->left.get(), strVars, arrVars) ||
                                 (bin->left->type == ASTNodeType::IDENTIFIER_EXPR &&
                                  strVars.count(static_cast<const IdentifierExpr*>(bin->left.get())->name));
            const bool rightStr = isFreshStringExpr(bin->right.get(), strVars, arrVars) ||
                                  (bin->right->type == ASTNodeType::IDENTIFIER_EXPR &&
                                   strVars.count(static_cast<const IdentifierExpr*>(bin->right.get())->name));
            if (leftStr || rightStr)
    ...
    case ASTNodeType::CALL_EXPR: {
        ...
        if (isKnownFreshStringBuiltin(call->callee))
            return true;
...
static bool isFreshArrayExpr(const Expression* expr, const std::unordered_set<std::string>& strVars,
    ...
    case ASTNodeType::CALL_EXPR: {
        const auto* call = static_cast<const CallExpr*>(expr);
        if (isKnownFreshArrayBuiltin(call->callee))
            return true;
        return false;
    }
    case ASTNodeType::TERNARY_EXPR: {
...
static void collectStringArrayVars(const Statement* stmt, std::unordered_set<std::string>& strVars,
...
static void markSharedVars(const Statement* stmt, const std::unordered_set<std::string>& strVars,
    ...
    const auto markArgShared = [&](const Expression* argExpr, const std::string& callee) {
        if (!argExpr)
            return;
        if (argExpr->type != ASTNodeType::IDENTIFIER_EXPR)
            return;
        ...
        if (isKnownNonCapturingBuiltin(callee))
            return;
    ...
    if (const auto* ps = dynamic_cast<const PrefetchStmt*>(stmt)) {
        if (ps->varDecl && ps->varDecl->initializer && ps->varDecl->initializer->type == ASTNodeType::CALL_EXPR) {
    ...
    if (const auto* fs = dynamic_cast<const ForStmt*>(stmt)) {
        ...
        if (fs->step)
            scanExprForAliases(fs->step.get());
    ...
    if (const auto* sw = dynamic_cast<const SwitchStmt*>(stmt)) {
        ...
        for (const auto& sc : sw->cases) {
            if (sc.value)
                scanExprForAliases(sc.value.get());
            for (const auto& val : sc.values)
                scanExprForAliases(val.get());
            for (const auto& s : sc.body)
                markSharedVars(s.get(), strVars, arrVars, sharedVars);
        }
        return;
    ...
    if (const auto* as = dynamic_cast<const AssumeStmt*>(stmt)) {
        if (as->condition)
            scanExprForAliases(as->condition.get());
        if (as->deoptBody)
...
UniquenessAnalysis computeUniqueness(const FunctionDecl& fn, const std::unordered_set<std::string>& strVarHints,
    ...
    for (const auto& param : fn.parameters) {
        if (strVars.count(param.name) || arrVars.count(param.name) || param.typeName == "string" ||
            param.typeName == "str" || (!param.typeName.empty() && param.typeName.back() == ']')) {
