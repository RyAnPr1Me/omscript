...
BuildSystem::BuildSystem(std::string projectDir, std::string profileName)
    : projectDir_(std::move(projectDir)), profileName_(std::move(profileName)) {}
...
bool BuildSystem::prepare(const BuildIO& io) {
    ...
    const auto manifestPath = (fs::path(projectDir_) / "oms.toml").string();
    if (fs::exists(manifestPath, ec) && !ec) {
        try {
            manifest_ = parseOmsToml(manifestPath);
        } catch (const std::exception& ex) {
            std::cerr << "Error: failed to parse oms.toml: " << ex.what() << "\n";
            return false;
        }
    }
    ...
    if (profIt == manifest_.profiles.end()) {
        ...
        for (const auto& [n, _] : manifest_.profiles)
            std::cerr << " " << n;
        std::cerr << "\n";
        return false;
    }
    ...
    const auto entryPath = (fs::path(projectDir_) / manifest_.entry).string();
    graph_.load(entryPath, projectDir_);

    if (graph_.empty() && !io.quiet) {
        std::cerr << "Warning: entry file '" << manifest_.entry << "' not found relative to project root '"
                  << projectDir_ << "'.\n";
    }
...
BuildResult BuildSystem::build(const BuildIO& io) {
    ...
    if (ec) {
        result.errorMessage = "Cannot create target directory '" + targetDir_ + "': " + ec.message();
        return result;
    }
    ...
    if (!io.quiet) {
        std::cout << "   Compiling " << manifest_.name << " v" << manifest_.version << " (" << profileName_ << ")\n";
    }
    ...
    if (!io.quiet) {
        std::cout << "    Finished `" << profileName_ << "` profile [optimized] target(s) in "
                  << static_cast<int>(ms / 1000.0) << "." << static_cast<int>(ms) % 1000 << "s\n";
    }
...
bool BuildSystem::clean(const BuildIO& io) {
    ...
    if (dir.empty()) {
        const auto profIt = manifest_.profiles.find(profileName_);
        if (profIt != manifest_.profiles.end() && !profIt->second.outDir.empty()) {
    ...
    if (ec) {
        std::cerr << "Error: cannot remove '" << dir << "': " << ec.message() << "\n";
        return false;
    }
    if (!io.quiet)
...
bool BuildSystem::runCompiler(const std::string& entryPath, const std::string& outPath, const BuildIO& io) {
    Compiler compiler;
    ...
    if (!marchCpu_.empty())
        compiler.setMarch(marchCpu_);
    if (!mtuneCpu_.empty())
        compiler.setMtune(mtuneCpu_);
    if (!pgoGenPath_.empty())
        compiler.setPGOGen(pgoGenPath_);
    if (!pgoUsePath_.empty())
        compiler.setPGOUse(pgoUsePath_);
