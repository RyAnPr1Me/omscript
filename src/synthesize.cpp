...
int64_t SynthesisEngine::eval(const SynthNode* node, const std::vector<int64_t>& inputs) {
    if (!node)
        return 0;
    switch (node->op) {
    case SynthOp::PARAM:
    ...
    case SynthOp::SHL: {
        ...
        if (shift < 0)
            shift = 0;
        return static_cast<int64_t>(static_cast<uint64_t>(eval(node->left.get(), inputs))
    ...
    case SynthOp::POW: {
        ...
        if (exp < 0)
            return (base == 1) ? 1 : 0;
        if (exp > 30)
            exp = 30; // guard against huge exponents
        int64_t result = 1;
        while (exp > 0) {
            if (exp & 1)
                result *= base;
...
bool SynthesisEngine::verifyAll(const SynthNode* node, const std::vector<SynthExample>& examples) {
...
void SynthesisEngine::enumerate(int maxDepth, int nParams, const std::vector<SynthOp>& allowedOps,
    ...
    auto fingerprint = [&](const SynthNode* n) -> std::string {
        ...
        char buf[32];
        std::snprintf(buf, sizeof(buf), "%lld|%lld", static_cast<long long>(r1), static_cast<long long>(r2));
        return buf;
    };
    ...
    for (int d = 1; d <= maxDepth; ++d) {
        for (SynthOp op : allowedOps) {
            if (isUnaryOp(op)) {
                ...
                for (int d1 = 0; d1 < d && static_cast<int>(byDepth[static_cast<size_t>(d)].size()) < kMaxPerLevel;
                     ++d1) {
                    for (int d2 = 0; d2 < d && static_cast<int>(byDepth[static_cast<size_t>(d)].size()) < kMaxPerLevel;
                         ++d2) {
                        if (std::max(d1, d2) != d - 1)
                            continue;
                        const auto& lhsVec = byDepth[static_cast<size_t>(d1)];
                        const auto& rhsVec = byDepth[static_cast<size_t>(d2)];
                        for (size_t li = 0; li < lhsVec.size() &&
                                            static_cast<int>(byDepth[static_cast<size_t>(d)].size()) < kMaxPerLevel;
                             ++li) {
                            for (size_t ri = 0; ri < rhsVec.size() &&
                                                static_cast<int>(byDepth[static_cast<size_t>(d)].size()) < kMaxPerLevel;
                                 ++ri) {
                                ...
                                const bool commutative =
                                    (op == SynthOp::ADD || op == SynthOp::MUL || op == SynthOp::AND ||
                                     op == SynthOp::OR || op == SynthOp::XOR || op == SynthOp::MIN2 ||
                                     op == SynthOp::MAX2);
                                if (commutative && d1 == d2 && li > ri)
                                    continue;
                                auto n = std::make_unique<SynthNode>(op);
...
std::optional<SynthResult> SynthesisEngine::synthesize(int nParams, const std::vector<SynthExample>& examples,
                                                       const SynthConfig& cfg) const {

    if (examples.empty() || nParams < 0)
        return std::nullopt;
    ...
    for (auto& cand : candidates) {
        if (checked >= cfg.maxCandidates)
            break;
        ++checked;

        if (!verifyAll(cand.get(), examples))
            continue;
...
std::string SynthesisEngine::render(const SynthNode* node, const std::vector<std::string>& paramNames) {
    if (!node)
        return "0";
    switch (node->op) {
    case SynthOp::PARAM: {
    ...
    case SynthOp::NOT:
        return "(~" + render(node->left.get(), paramNames) + ")";
    case SynthOp::ADD:
        return "(" + render(node->left.get(), paramNames) + " + " + render(node->right.get(), paramNames) + ")";
    case SynthOp::SUB:
        return "(" + render(node->left.get(), paramNames) + " - " + render(node->right.get(), paramNames) + ")";
    case SynthOp::MUL:
        return "(" + render(node->left.get(), paramNames) + " * " + render(node->right.get(), paramNames) + ")";
    ...
    case SynthOp::POW:
        return "(" + render(node->left.get(), paramNames) + " ** " + render(node->right.get(), paramNames) + ")";
    case SynthOp::MIN2:
        return "min(" + render(node->left.get(), paramNames) + ", " + render(node->right.get(), paramNames) + ")";
    case SynthOp::MAX2:
        return "max(" + render(node->left.get(), paramNames) + ", " + render(node->right.get(), paramNames) + ")";
    }
    return "0";
}
...
std::unique_ptr<Expression> SynthesisEngine::lowerToAST(const SynthNode* node,
    ...
    case SynthOp::ADD:
        return makeBinary("+", lowerToAST(node->left.get(), paramNames), lowerToAST(node->right.get(), paramNames));
    case SynthOp::SUB:
        return makeBinary("-", lowerToAST(node->left.get(), paramNames), lowerToAST(node->right.get(), paramNames));
    case SynthOp::MUL:
        return makeBinary("*", lowerToAST(node->left.get(), paramNames), lowerToAST(node->right.get(), paramNames));
...
static bool extractSynthesisArgs(const CallExpr* call, std::vector<SynthExample>& examples, SynthConfig& cfg) {
    ...
    for (const auto& innerExpr : examplesArg->elements) {
        ...
        if (!innerArr || innerArr->elements.size() < 2)
            return false;
...
void runSynthesisPass(Program* program, bool verbose) {
    ...
    for (auto& fn : program->functions) {
        ...
        if (!extractSynthesisArgs(synthCall, examples, cfg)) {
            if (verbose)
                std::cerr << "  [synthesize] Warning: could not extract examples from " << fn->name << "()\n";
            ++failed;
            continue;
        }
        ...
        if (!examples.empty() && static_cast<size_t>(nParams) != examples[0].inputs.size()) {
            if (verbose)
                std::cerr << "  [synthesize] Warning: example input count (" << examples[0].inputs.size()
                          << ") doesn't match parameter count (" << nParams << ") in " << fn->name << "()\n";
            ++failed;
            continue;
        }

        if (verbose)
            std::cout << "  [synthesize] Synthesizing body for " << fn->name << "() (" << examples.size()
                      << " examples, " << nParams << " params, depth≤" << cfg.maxDepth << ")...\n";

        auto result = engine.synthesize(nParams, examples, cfg);
        if (!result) {
            if (verbose)
                std::cerr << "  [synthesize] Warning: no expression found for " << fn->name
                          << "() — body left unchanged\n";
            ++failed;
            continue;
        }
            ...
            std::cout << "  [synthesize] Found: " << fn->name << "(" <<
                [&]() {
                    ...
                    return s;
                      ...
                      << ") = " << SynthesisEngine::render(result->expr.get(), paramNames) << "  [cost=" << result->cost
                      << ", nodes=" << result->nodes << ", depth=" << result->depth << "]\n";
    ...
    if (verbose && (synthesized + failed) > 0) {
        std::cout << "  [synthesize] Pass complete: " << synthesized << " synthesized, " << failed << " failed\n";
    }
}
